...
class AnalyticsFrame(ctk.CTkScrollableFrame):
    def __init__(self, parent, controller):
        ...
        self._active_tab = 0
        self.servico_analytics = ServicoAnalytics(auth_service=getattr(controller, 'auth_service', None))
    ...
    def _on_search(self, query: str) -> None:
        ...
        def fetch() -> dict:
            return self.servico_analytics.buscar_global(query)
    ...
    def _render_search_results(self, result) -> None:
        ...
        ctk.CTkLabel(
            container, text=f"{len(items)} resultado(s) encontrado(s)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_muted"],
