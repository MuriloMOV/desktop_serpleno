...
class PageHeader(ctk.CTkFrame):
    ...
    def _build(self, title, subtitle, actions, show_breadcrumb, breadcrumb_parts):
        ...
        if subtitle:
            ctk.CTkLabel(
                txt, text=subtitle, font=themed_font("body"), text_color=THEME["text_muted"]
...
class SectionHeader(ctk.CTkFrame):
    ...
    def _build(self, title, subtitle, action_text, action_command, icon):
        ...
        if subtitle:
            ctk.CTkLabel(
                left, text=subtitle, font=themed_font("body_sm"), text_color=THEME["text_muted"]
            ).pack(anchor="w", pady=(2, 0))
...
class Card(ctk.CTkFrame):
    ...
    Usa dois frames aninhados para evitar bug do CustomTkinter 5.2.2
    onde border_width>0 + corner_radius>0 causa artefatos e altura excessiva.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Frame externo: apenas borda (cor da borda como fg_color), sem corner_radius no conteúdo
        super().__init__(
            parent,
        ...
        # Frame interno: conteúdo com background da superfície, corner_radius reduzido
        inner_radius = max(0, RADIUS["card"] - 1)
    ...
    def _build_with_title(self, title):
        ...
        ctk.CTkFrame(self._inner, height=1, fg_color=THEME["divider"]).pack(
            fill="x", padx=self._padding[0], pady=(10, 0)
        )
        self._body = ctk.CTkFrame(self._inner, fg_color="transparent")
        self._body.pack(fill="x", padx=self._padding[0], pady=(10, self._padding[1]))
    ...
    def _build_with_status(self, title, status):
        ...
        status_pill = ctk.CTkFrame(
            header, fg_color=blend_color(color, 0.12), corner_radius=RADIUS["pill"], height=24
        )
        ...
        self._body = ctk.CTkFrame(self._inner, fg_color="transparent")
        self._body.pack(fill="x", padx=self._padding[0], pady=(0, self._padding[1]))

    def _build_empty(self):
        self._body = ctk.CTkFrame(self._inner, fg_color="transparent")
        self._body.pack(fill="x", padx=self._padding[0], pady=self._padding[1])
    ...
    @body.setter
    def body(self, value: ctk.CTkFrame) -> None:
        self._body = value

    @property
    def footer(self):
        if not hasattr(self, "_footer"):
            self._footer = ctk.CTkFrame(self._inner, fg_color="transparent")
...
class KPICard(ctk.CTkFrame):
    ...
    def _build(self, title, value, icon, trend, unit):
        ...
        ctk.CTkLabel(
            txt,
            text=title,
            font=themed_font(self._size_cfg["title"]),
        ...
        self._value_label = ctk.CTkLabel(
            value_frame,
            text=value,
            font=themed_font(self._size_cfg["value"], "bold"),
        ...
        if unit:
            ctk.CTkLabel(
                value_frame,
                text=unit,
                font=themed_font(self._size_cfg["sub"]),
                text_color=THEME["text_muted"],
        ...
        if trend:
            ...
            trend_color = (
                THEME["success"]
                if trend_up
                else (THEME["danger"] if trend_down else THEME["text_muted"])
            )
            ctk.CTkLabel(
                txt,
                text=trend,
                font=themed_font(self._size_cfg["sub"], "bold"),
        ...
        icon_container = ctk.CTkFrame(
            row1, fg_color=icon_bg, width=icon_size, height=icon_size, corner_radius=RADIUS["lg"]
        )
        icon_container.pack(side="right")
        icon_container.pack_propagate(False)
        ctk.CTkLabel(
            icon_container, text=icon, font=themed_font("h2"), text_color=self.accent
        ).place(relx=0.5, rely=0.5, anchor="center")

        if self._size == "wide":
            self._pbar = ctk.CTkProgressBar(
                content, height=4, progress_color=self.accent, fg_color=THEME["bg_alt"]
            )
...
class MetricCard(ctk.CTkFrame):
    ...
    def _build(self, title, value, change, change_type, icon, sparkline):
        ...
        ctk.CTkLabel(
            hdr, text=title, font=themed_font("body_sm"), text_color=THEME["text_muted"]
        ).pack(side="left")
        ...
        ctk.CTkLabel(
            val_f, text=value, font=themed_font("h1", "bold"), text_color=THEME["text"]
        ).pack(side="left")

        ch_color = (
            THEME["success"]
            if change_type == "positive"
            else (THEME["danger"] if change_type == "negative" else THEME["text_muted"])
        )
        if change:
            ctk.CTkLabel(
                val_f, text=change, font=themed_font("body", "bold"), text_color=ch_color
            ).pack(side="left", padx=(12, 0), pady=(6, 0))

        if sparkline and len(sparkline) > 1:
            canvas = ctk.CTkCanvas(content, height=48, bg=THEME["surface"], highlightthickness=0)
            canvas.pack(fill="x")
...
class ListCard(ctk.CTkFrame):
    ...
    def _build(self, title, description, icon, status, status_color, avatar_text, avatar_color):
        inner = ctk.CTkFrame(self._inner, fg_color="transparent")
        inner.pack(fill="both", expand=True, padx=SPACING["card_pad"], pady=SPACING["card_pad"])
        inner.grid_columnconfigure(1, weight=1)

        if avatar_text:
            av = ctk.CTkFrame(
                inner, fg_color=avatar_color, width=44, height=44, corner_radius=RADIUS["avatar"]
            )
            av.grid(row=0, column=0, rowspan=2, padx=(0, 14))
            av.pack_propagate(False)
            ctk.CTkLabel(
                av, text=avatar_text, font=themed_font("h3", "bold"), text_color=THEME["primary"]
            ).place(relx=0.5, rely=0.5, anchor="center")

        title_frame = ctk.CTkFrame(inner, fg_color="transparent")
        title_frame.grid(row=0, column=1, sticky="ew")
        ctk.CTkLabel(
            title_frame, text=title, font=themed_font("h4", "bold"), text_color=THEME["text"]
        ).pack(side="left")
        ...
        if status:
            sp = ctk.CTkFrame(
                inner,
                fg_color=blend_color(status_color, 0.12),
            ...
            ctk.CTkLabel(
                sp, text=status, font=themed_font("overline", "bold"), text_color=status_color
            ).pack(padx=10, pady=2)

        if description:
            ctk.CTkLabel(
                inner, text=description, font=themed_font("body_sm"), text_color=THEME["text_muted"]
            ).grid(row=1, column=1, sticky="w", pady=(4, 0))


# ••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
#  Linhas de lista e indicadores
# ••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
class ListRow(ctk.CTkFrame):
    """Linha de lista genérica com barra de cor, título, subtítulo e badge à direita."""
    ...
    def __init__(
        ...
        self._build(title, subtitle, color, soft_color, trailing_badge, icon)

    def _build(self, title, subtitle, color, soft_color, trailing_badge, icon):
        if color:
            ...
            color_bar.pack(side="left", padx=(spacing("sm"), spacing("md")), pady=4)
        ...
        if trailing_badge:
            ...
            time_frame.pack(side="right", padx=(spacing("md"), spacing("md")), pady=4)
            time_frame.pack_propagate(False)
            ctk.CTkLabel(
                time_frame,
                text=f"{icon}  {trailing_badge}" if icon else trailing_badge,
                font=themed_font("body", "bold"),
                text_color=color,
...
class AlertRow(ctk.CTkFrame):
    """Linha de alerta com barra de cor por prioridade, nome, motivo e badge."""
    ...
    _PRIORITY_ICON = {
        ...
        3: ICONS["priority_high"],
    }

    def __init__(self, parent, name: str, reason: str = "", priority: int = 0, color: str = ""):
        bar_color = color or self._PRIORITY_COLOR.get(priority, THEME["warning"])
        super().__init__(
            parent,
            fg_color=THEME["bg_alt"],
            corner_radius=RADIUS["md"],
        ...
        self.pack_propagate(False)
        self._build(name, reason, priority, bar_color)

    def _build(self, name, reason, priority, bar_color):
        color_bar = ctk.CTkFrame(
            self,
            width=spacing("xs"),
            height=28,
        ...
        label = self._PRIORITY_LABEL.get(priority, "")
        chip = ctk.CTkFrame(
            self,
        ...
        chip.pack(side="right", padx=(spacing("md"), spacing("md")), pady=4)
        chip.pack_propagate(False)
        ctk.CTkLabel(
            chip,
            text=f"{self._PRIORITY_ICON[priority]}  {label}",
            font=themed_font("caption", "bold"),
            text_color=THEME["danger"],
...
class ProgressBar(ctk.CTkFrame):
    """Barra de progresso com rótulo, porcentagem e preenchimento visual."""
    ...
    def _build(self, nome, valor, color, soft):
        ...
        bar_bg = ctk.CTkFrame(
            self, fg_color=THEME["chart_grid"], corner_radius=RADIUS["pill"], height=7
        )
        bar_bg.pack(fill="x", pady=(6, 0))
        bar_bg.pack_propagate(False)

        fill_w = max(1, int(valor * 1000))
...
class SummaryCard(ctk.CTkFrame):
    """Cartão de resumo com ícone, valor, título e barra de cor inferior."""

    def __init__(self, parent, title: str, icon: str, accent: str, soft: str, sub: str = ""):
        super().__init__(
            parent,
            fg_color=THEME["surface"],
            corner_radius=RADIUS["card"],
        ...
        icon_bg = ctk.CTkFrame(
            top, width=44, height=44, corner_radius=RADIUS["button"], fg_color=soft
        )
        icon_bg.pack(side="left")
        icon_bg.pack_propagate(False)
        ctk.CTkLabel(icon_bg, text=icon, font=themed_font("h3")).place(
...
class InputField(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.entry.bind(
            "<FocusIn>",
            lambda e: self._inner.configure(
                border_color=THEME["input_border_focus"], border_width=2
            ),
        )
        self.entry.bind(
            "<FocusOut>",
            lambda e: self._inner.configure(border_color=THEME["input_border"], border_width=1),
        )
    ...
    def _show_error(self, msg: str):
        ...
        err = ctk.CTkLabel(self, text=msg, font=themed_font("overline"), text_color=THEME["danger"])
...
class SearchField(ctk.CTkFrame):
    ...
    def _build(self, placeholder):
        ...
        ctk.CTkLabel(
            self, text=ICONS["search"], font=themed_font("body"), text_color=THEME["text_muted"]
...
class Badge(ctk.CTkFrame):
    ...
    def __init__(
        ...
        ctk.CTkLabel(
            self, text=str(text), font=themed_font("overline", "bold"), text_color=color
        ).pack(padx=10, pady=4)
        if outline:
...
class Pill(ctk.CTkFrame):
    ...
    def __init__(
        ...
        ctk.CTkLabel(self, text=text, font=themed_font("overline", "bold"), text_color=tc).pack(
            padx=10, pady=3
        )
...
class Toast(ctk.CTkFrame):
    ...
    def __init__(
        ...
        icon_lbl = ctk.CTkLabel(
            inner, text=icons.get(status, "ℹ"), font=themed_font("h3"), text_color=c
        )
        icon_lbl.pack(side="left", padx=(0, 10))

        lbl = ctk.CTkLabel(
            inner, text=message, font=themed_font("body"), text_color=THEME["text"], anchor="w"
        )
...
class Tabs(ctk.CTkFrame):
    ...
    def __init__(
        self, parent, tabs: list[str], on_select: Optional[Callable] = None, initial: int = 0
    ):
        super().__init__(parent, fg_color="transparent")
        self.tabs = tabs
...
class Toggle(ctk.CTkSwitch):
    ...
    def __init__(self, parent, text: str, command=None, initial: bool = False):
        ...
        self._lbl = ctk.CTkLabel(
            parent, text=text, font=themed_font("body"), text_color=THEME["text"]
        )
...
class Avatar(ctk.CTkFrame):
    ...
    def __init__(
        ...
        super().__init__(
            parent, fg_color=color, width=size, height=size, corner_radius=RADIUS["avatar"]
        )
        self.pack_propagate(False)
        half = size // 2
        ctk.CTkLabel(
        ...
        if status:
            self._status = ctk.CTkFrame(
                self,
                fg_color=STATUS_COLORS.get(status.lower(), THEME["success"]),
                width=max(10, size // 5),
...
class SegmentedButton(ctk.CTkFrame):
    ...
    def __init__(
        self, parent, options: list[str], on_change: Optional[Callable] = None, initial: int = 0
    ):
...
class SectionCard(Card):
    def __init__(self, parent, title: str = "", action_text: str = "", action_cmd=None, **kwargs):
        kwargs.setdefault("auto_body", False)
        ...
        Divider(self._inner).pack(fill="x", padx=SPACING["card_pad"], pady=(0, SPACING["label_gap"]))
        self.body = ctk.CTkFrame(self._inner, fg_color="transparent")
        self.body.pack(fill="both", expand=True, padx=SPACING["card_pad"], pady=(0, SPACING["card_pad"]))
...
class FormField(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        col = 0
        if self._icon and not self._values:
            ctk.CTkLabel(
                self._box,
                text=self._icon,
                font=font(size=14),
        ...
        if self._values is not None:
            self.widget = ctk.CTkComboBox(
                ...
                font=font(size=13),
                height=self._height or 38,
            )
            if self._initial:
                self.widget.set(self._initial)
            self.widget.grid(
                row=0,
                column=col,
                columnspan=2 - col,
                sticky="ew",
                padx=(8 if col == 0 else 4, 8),
                pady=4,
            )
        elif self._multiline:
            self.widget = ctk.CTkTextbox(
                self._box,
                height=self._height or 100,
                fg_color=THEME["input_bg"],
                border_width=0,
                font=font(size=13),
                corner_radius=0,
            ...
            if self._initial:
                self.widget.insert("1.0", self._initial)
            self.widget.grid(
                row=0,
                column=col,
                columnspan=2 - col,
            ...
            self.widget = ctk.CTkEntry(
                ...
                text_color=THEME["text"],
                font=font(size=13),
                height=self._height or 40,
                show="●" if self._password else "",
            )
            if self._initial:
                self.widget.insert(0, self._initial)
            self.widget.grid(
                row=0,
                column=col,
                columnspan=2 - col,
        ...
        self.entry = self.widget if isinstance(self.widget, ctk.CTkEntry) else None
        ...
        if self.helper_text:
            self._err_lbl.pack(fill="x", pady=(3, 0))
    ...
    def set_error(self, msg: str = "") -> None:
        self._error = msg
        self._box.configure(
            border_color=THEME["danger"], fg_color=THEME["danger_soft"]
        )
        self._label.configure(text_color=THEME["danger"])
        if hasattr(self, "_err_lbl"):
            self._err_lbl.configure(
                text=f"{ICONS['bolt']}   {msg}" if msg else self.helper_text,
                text_color=THEME["danger"],
            )
        elif msg:
            err = ctk.CTkLabel(
                self,
                text=f"{ICONS['bolt']}   {msg}",
                font=font(size=11),
                text_color=THEME["danger"],
                anchor="w",
            )
            err.pack(fill="x", pady=(3, 0))

    def clear_error(self) -> None:
        self._error = ""
        self._box.configure(
            border_color=THEME["input_border"], fg_color=THEME["input_bg"]
        )
        self._label.configure(text_color=THEME["text_secondary"])
        if hasattr(self, "_err_lbl"):
            self._err_lbl.configure(text=self.helper_text, text_color=THEME["text_secondary"])
    ...
    def _on_focus_in(self, _=None) -> None:
        self._box.configure(
            border_color=THEME["input_border_focus"], fg_color=THEME["surface"]
        )
...
class Chip(ctk.CTkFrame):
    def __init__(self, parent, text: str, fg_color: Optional[str] = None, text_color: Optional[str] = None, corner_radius: Optional[int] = None):
        fg_color = fg_color or THEME["border"]
        ...
        ctk.CTkLabel(self, text=text, font=themed_font("body_sm", "bold"), text_color=text_color).pack(
            padx=spacing("sm"), pady=spacing("xs")
        )


def clear_children(widget: Any) -> None:
