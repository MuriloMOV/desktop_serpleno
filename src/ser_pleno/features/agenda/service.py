    ...
    def criar_agendamento(self, dados):
        ...
        try:
            ...
            try:
                ...
                try:
                    self._sync_with_serpleno_web(
                        appointment_id, id_aluno, data_hora, nome_agendamento, dados
                    )
                except Exception as sync_error:
                    logging.warning(
                        f"Erro ao sincronizar com Serpleno Web (não bloqueante): {sync_error}"
                    )
    ...
    def _sync_with_serpleno_web(self, appointment_id, student_id, data_hora, nome, dados):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logging.warning(
                    f"Falha ao sincronizar agendamento {appointment_id}: status {response.status_code}"
                )
        except Exception as e:
                    ...
                    for apt in result["data"]["appointments"]:
                        ...
                        if data_hora_str and time_str:
                            data_hora = datetime.strptime(
                                f"{data_hora_str} {time_str}", "%Y-%m-%d %H:%M"
                            )
                        else:
                            data_hora = None
    ...
    def _sync_with_api(self, appointment_id):
        ...
        try:
            ...
            payload = {
                ...
                if hasattr(agendamento["data_hora"], "isoformat")
                else str(agendamento["data_hora"]),
