...
class _Field(ctk.CTkFrame):
    ...
    def set_error(self, msg: str):
        self._err.configure(text=f"âš   {msg}", text_color=THEME["danger"])
...
class EstudantesFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self._item_widgets: dict = {}   # id â†’ frame widget
    ...
    def _criar_painel_detalhes(self, parent):
        ...
        self.tab_intervencoes = self.tabs.add("IntervençÓµes")
    ...
    def _construir_tab_perfil(self):
        ...
        cfg = [
            ("Contato",       "--", "ðŸ“§", 0, 0, "card_email"),
            ("Idade",         "--", "ðŸŽ‚", 0, 1, "card_idade"),
            ("Curso / Turma", "--", "ðŸŽ“", 1, 0, "card_curso"),
            ("Laudo Médico",  "--", "ðŸ“„", 1, 1, "card_laudo"),
        ]
    ...
    def _construir_tab_intervencoes(self):
        ...
        ctk.CTkLabel(
            self.tab_int_inner,
            text="Selecione um estudante para ver as intervençÓµes",
            font=themed_font("body"),
            text_color=THEME["text_secondary"],
        ).pack(pady=30)
    ...
    def _renderizar_estudantes(self, lista: list):
        ...
        if not lista:
            ctk.CTkLabel(
                self.scroll_list,
                text="ðŸ˜•  Nenhum estudante encontrado",
                font=themed_font("body"),
                text_color=THEME["text_secondary"],
            ).pack(pady=24)
    ...
    def selecionar_estudante(self, st: dict, widget=None):
        ...
        if self.card_laudo:
            self.card_laudo.configure(
                text="âœ”  Sim" if st.get("has_medical_report") else "âœ—  Não",
    ...
    def novo_estudante_click(self):
        ...
        en_nome = _Field(body, "Nome Completo", "Ex: Ana Silva", icon="ðŸ‘¤",
                         helper="Nome completo do estudante")
        en_nome.pack(fill="x", pady=(0, 12))

        en_email = _Field(body, "Email de Contato", "email@exemplo.com", icon="ðŸ“§",
        ...
        en_curso = _Field(row_mid, "Curso / Turma", "Ex: Psicologia", icon="ðŸŽ“")
        en_curso.grid(row=0, column=0, sticky="ew", padx=(0, 8))
        en_idade = _Field(row_mid, "Idade", "Ex: 22", icon="ðŸŽ‚")
        ...
        sw_laudo = self._switch_row(
            sw_frame, "ðŸ“„", "Possui laudo médico",
        ...
        sw_aten = self._switch_row(
            sw_frame, "âš ", "Requer atendimento prioritário",
        ...
        PrimaryButton(
            footer, text="âœ”  Salvar Estudante",
            command=salvar,
            height=38, width=180,
