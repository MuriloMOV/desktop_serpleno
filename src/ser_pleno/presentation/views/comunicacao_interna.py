...
_AVATAR_INITIALS = {
    ...
    "coordenador": "CO", "suporte": "SP", "group": "ðŸ‘¥",
}

# Ócones de tipo de arquivo
_FILE_ICONS = {
    "Imagens": "ðŸ–¼ï¸", "Videos": "ðŸŽ¥", "Audio": "ðŸŽµ",
    "Planilhas": "ðŸ“Š", "PresentaçÓµes": "ðŸ“½ï¸",
    "Arquivos Zip": "ðŸ—œï¸", "Code": "ðŸ’»",
}
...
class ComunicacaoInternaFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self._images: dict = {}
        self._contact_widgets: dict = {}   # id â†’ frame widget
    ...
    def _criar_contato_item(self, contato: dict):
        ...
        av_init  = nome[:2].upper() if papel != "group" else "ðŸ‘¥"
    ...
    def _criar_chat_header(self, parent):
        ...
        # BotÓµes de ação (direita)
        actions = ctk.CTkFrame(inner, fg_color="transparent")
        actions.pack(side="right")

        for icon in ("ðŸ“·", "ðŸ“ž", "â‹®"):
            ctk.CTkButton(
                actions, text=icon,
    ...
    def _criar_modal_arquivos(self, parent):
        ...
        categorias = [
            ("ðŸ“„", "Documentos",   [".pdf", ".doc", ".docx", ".txt"]),
            ("ðŸ–¼ï¸", "Imagens",      [".jpg", ".jpeg", ".png", ".gif"]),
            ("ðŸŽ¥", "Vídeos",       [".mp4", ".avi", ".mov"]),
            ("ðŸŽµ", "Óudio",        [".mp3", ".wav", ".ogg"]),
            ("ðŸ“Š", "Planilhas",    [".xls", ".xlsx", ".csv"]),
            ("ðŸ“½ï¸", "ApresentaçÓµes",[".ppt", ".pptx"]),
            ("ðŸ—œï¸", "Compactados",  [".zip", ".rar", ".7z"]),
            ("ðŸ’»", "Código",       [".py", ".js", ".html", ".css"]),
            ("ðŸ“", "Todos",        []),
        ]
    ...
    def _criar_mensagem_arquivo(self, bubble, msg: dict, txt_color: str):
        ...
        icon   = _FILE_ICONS.get(msg.get("tipo_arquivo", ""), "ðŸ“„")
        ...
        for icon_btn, cmd in [
            ("ðŸ‘", lambda p=msg["caminho_arquivo"]: self.visualizar_arquivo(p)),
            ("ðŸ“¥", lambda p=msg["caminho_arquivo"], n=nome: self.download_arquivo(p, n)),
        ]:
            ctk.CTkButton(
                btns, text=icon_btn,
