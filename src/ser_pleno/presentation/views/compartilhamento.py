...
from ser_pleno.presentation.components.ui_components import (
    ...
    SkeletonLoader, EmptyState, KPICard, Toast, BaseModal,
)
...
class CompartilhamentoDadosFrame(ctk.CTkFrame):
    ...
    def _criar_toolbar_acoes(self):
        ...
        self.btn_compartilhar = PrimaryButton(
            right, text=f"{ICONS['send']}  Compartilhar",
        ...
        self.btn_descompartilhar = DangerButton(
            right, text=f"{ICONS['close']}  Descompartilhar",
        ...
        self.btn_bulk_share = PrimaryButton(
            right, text=f"{ICONS['group']}  Bulk Share",
        ...
        self.btn_bulk_unshare = DangerButton(
            right, text=f"{ICONS['group']}  Bulk Unshare",
    ...
    def _criar_sidebar(self, parent):
        ...
        self.f_tipo = ctk.CTkOptionMenu(
            f_row,
            values=["Todos", "medical_report", "academic_record", "attendance", "intervention", "psychological"],
        ...
        self.lbl_count = ctk.CTkLabel(
            sidebar, text="0 compartilhamentos",
            font=themed_font("caption"),
    ...
    def _criar_painel_principal(self, parent):
        ...
        self.tabs.grid(row=1, column=0, sticky="nsew", padx=SPACING["card_pad"], pady=(SPACING["item_gap"], 0))
    ...
    def _construir_tab_lista(self):
        self.tab_lista_inner = ctk.CTkScrollableFrame(
            self.tab_lista, fg_color="transparent",
            scrollbar_button_color=THEME["border_strong"],
            scrollbar_button_hover_color=THEME["text_muted"],
        )
    ...
    def _construir_tab_historico(self):
        self.tab_historico_inner = ctk.CTkScrollableFrame(
            self.tab_historico, fg_color="transparent",
            scrollbar_button_color=THEME["border_strong"],
            scrollbar_button_hover_color=THEME["text_muted"],
        )
    ...
    def _carregar_lista(self):
        def fetch():
            return self.controller.listar_compartilhamentos()
    ...
    def _carregar_relatorio(self):
        def fetch():
            return self.controller.obter_relatorio_compartilhamento()
    ...
    def _mostrar_skeletons_lista(self):
        ...
        for _ in range(6):
            batch.add(lambda: SkeletonLoader(self.tab_lista_inner, width=260, height=56, variant="card").pack(
    ...
    def _renderizar_compartilhamentos(self, lista: list):
        ...
        if not lista:
            EmptyState(
                self.tab_lista_inner, icon=ICONS["empty"], title="Nenhum compartilhamento encontrado",
                subtitle="Ajuste os filtros ou realize um novo compartilhamento"
            ).pack(pady=24)
            self.lbl_count.configure(text="0 compartilhamentos")
            return

        self.lbl_count.configure(text=f"{len(lista)} compartilhamento{'s' if len(lista) != 1 else ''}")
    ...
    def _criar_item_compartilhamento(self, comp: dict):
        ...
        ctk.CTkLabel(
            inner, text=student_name,
            font=themed_font("body", "bold"),
            text_color=THEME["text"], anchor="w",
        ).grid(row=0, column=1, sticky="w")

        ctk.CTkLabel(
            inner, text=f"{data_type} → {shared_with_name} ({shared_with_role})",
            font=themed_font("caption"),
        ...
        row.bind("<Enter>", lambda e, r=row, cid=comp.get("id"): r.configure(fg_color=THEME["primary_soft"])
                 if cid not in self._selecionados else None)
        row.bind("<Leave>", lambda e, r=row, c=comp: r.configure(
            fg_color=THEME["primary_soft"] if c.get("id") in self._selecionados else THEME["bg_alt"]
        ))
    ...
    def _abrir_modal_compartilhar(self):
        ...
        ctk.CTkLabel(card, text="Tipo de Dado",
                     font=themed_font("body"),
                     text_color=THEME["text_secondary"], anchor="w").pack(fill="x", pady=(0, 4))

        self.combo_tipo = ctk.CTkComboBox(
            card, values=["medical_report", "academic_record", "attendance", "intervention", "psychological"],
            fg_color=THEME["bg_alt"],
            border_color=THEME["border"],
            button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
        ...
        ctk.CTkLabel(card, text="Compartilhar com (usuário)",
                     font=themed_font("body"),
                     text_color=THEME["text_secondary"], anchor="w").pack(fill="x", pady=(0, 4))

        self.combo_usuario = ctk.CTkComboBox(
            card, values=["Selecione um usuário"],
            fg_color=THEME["bg_alt"],
            border_color=THEME["border"],
            button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
        ...
        ctk.CTkLabel(card, text="Função",
                     font=themed_font("body"),
                     text_color=THEME["text_secondary"], anchor="w").pack(fill="x", pady=(0, 4))

        self.entry_funcao = ctk.CTkEntry(
            card, placeholder_text="Ex: professor, psicólogo",
        ...
        def compartilhar():
            ...
            res = self.controller.compartilhar(dados)
            if res.get("success"):
                self._show_toast("Dados compartilhados com sucesso.", "success")
                modal.destroy()
                self._carregar_lista()
            else:
                self._show_toast(f"Falha ao compartilhar: {res.get('error', res.get('message', ''))}", "danger")

        PrimaryButton(
            footer, text=f"{ICONS['send']}  Compartilhar",
    ...
    def _abrir_modal_bulk_share(self):
        ...
        ctk.CTkLabel(card, text="Bulk Share",
                     font=themed_font("h3", "bold"),
                     text_color=THEME["text"]).pack(anchor="w", pady=(0, 16))
        ...
        def bulk_share():
            ...
            if self._selecionados:
                ...
                student_ids = [c.get("student", {}).get("id") for c in self._compartilhamentos if isinstance(c, dict)]
            ...
            res = self.controller.bulk_share(dados)
            if res.get("success"):
                ...
                self._show_toast(f"Falha ao realizar bulk share: {res.get('error', res.get('message', ''))}", "danger")

        PrimaryButton(
            footer, text=f"{ICONS['send']}  Bulk Share",
            command=bulk_share,
            height=38, width=180,
        ).pack(side="right", pady=13)
    ...
    def _abrir_modal_bulk_unshare(self):
        ...
        ctk.CTkLabel(card, text="Bulk Unshare",
                     font=themed_font("h3", "bold"),
                     text_color=THEME["text"]).pack(anchor="w", pady=(0, 16))
        ...
        def bulk_unshare():
            ...
            res = self.controller.bulk_unshare(dados)
            if res.get("success"):
                self._show_toast(res.get("message", "Bulk unshare realizado com sucesso."), "success")
                modal.destroy()
                self._selecionados.clear()
                self._atualizar_estado_botoes()
                self._carregar_lista()
            else:
                self._show_toast(f"Falha ao realizar bulk unshare: {res.get('error', res.get('message', ''))}", "danger")

        DangerButton(
            footer, text=f"{ICONS['close']}  Bulk Unshare",
    ...
    def _descompartilhar_selecionados(self):
        ...
        for comp_id in list(self._selecionados):
            ...
            if comp:
                ...
                res = self.controller.descompartilhar(dados)
                if not res.get("success"):
                    self._show_toast(f"Falha ao descompartilhar: {res.get('error', res.get('message', ''))}", "danger")
    ...
    def _carregar_combos(self, modal):
        def fetch():
            return self.controller.listar_estudantes_compartilhados()
            ...
            if result.get("success"):
                ...
                if isinstance(data, list):
                    ...
                    usuarios = [
                        ...
                        for s in data if s.get("id") is not None
                    ]

            valores_estudantes = [f"{s.get('id')} - {s.get('name', '')}" for s in estudantes]
            self.combo_estudante.configure(values=valores_estudantes or ["Nenhum estudante encontrado"])
    ...
    def _carregar_combos_bulk(self, modal):
        ...
        def on_success(result):
            ...
            valores_usuarios = [f"{u.get('id')} - {u.get('name', '')}" for u in usuarios]
            self.bulk_combo_usuario.configure(values=valores_usuarios or ["Nenhum usuário encontrado"])
    ...
    def _carregar_combos_bulk_unshare(self, modal):
        ...
        def on_success(result):
            ...
            valores_usuarios = [f"{u.get('id')} - {u.get('name', '')}" for u in usuarios]
            self.bulk_unshare_combo_usuario.configure(values=valores_usuarios or ["Nenhum usuário encontrado"])
    ...
    def _carregar_historico(self, student_id: int):
        ...
        def fetch():
            return self.controller.obter_historico_compartilhamento(student_id)
        ...
        def on_success(result):
            ...
            if not history:
                EmptyState(
                    self.tab_historico_inner, icon=ICONS["empty"], title="Nenhum histórico encontrado",
    ...
    def _criar_item_historico(self, h: dict):
        ...
        ctk.CTkLabel(
            inner, text=f"{action.capitalize()} - {data_type}",
            font=themed_font("body", "bold"),
            text_color=THEME["text"], anchor="w",
        ).grid(row=0, column=1, sticky="w")

        ctk.CTkLabel(
            inner, text=f"De: {shared_by.get('name', '??')} Para: {shared_with.get('name', '??')} ({shared_with.get('role', '')})",
            font=themed_font("caption"),
            text_color=THEME["text_secondary"], anchor="w",
        ).grid(row=1, column=1, sticky="w", pady=(2, 0))
    ...
    def render_relatorio(self, result):
        ...
        if not report:
            EmptyState(
                self.tab_relatorio_inner, icon=ICONS["empty"], title="Nenhum dado de relatório",
        ...
        KPICard(kpi_row, title="Total Compartilhamentos", value=str(total),
                icon=ICONS["send"], size="md").grid(row=0, column=0, padx=(0, 8), sticky="nsew")
        KPICard(kpi_row, title="Estudantes", value=str(total_estudantes),
                icon=ICONS["group"], size="md").grid(row=0, column=1, padx=(8, 8), sticky="nsew")
        KPICard(kpi_row, title="Usuários Compartilhados", value=str(total_usuarios),
                icon=ICONS["users"], size="md").grid(row=0, column=2, padx=(8, 0), sticky="nsew")
