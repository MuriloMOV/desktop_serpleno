...
from ser_pleno.presentation.components.ui_components import (
    Divider, EmptyState, GhostButton, Card, Avatar, KPICard,
    PrimaryButton, DangerButton, SearchField, Tabs, Badge, Pill, Toast, BaseModal,
)
...
class _MoodEntryModal(ctk.CTkToplevel):
    ...
    def _build(self, student_id, student_name):
        ...
        self._en_student = ctk.CTkEntry(
            row1, placeholder_text="Estudante",
        ...
        ctk.CTkLabel(
            energy_frame, text="Nível de Energia (1–5)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._sl_energy = ctk.CTkSlider(
            energy_frame, from_=1, to=5, number_of_steps=4,
            progress_color=THEME["primary"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
        )
        ...
        ctk.CTkLabel(
            stress_frame, text="Nível de Estresse (1–5)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._sl_stress = ctk.CTkSlider(
            stress_frame, from_=1, to=5, number_of_steps=4,
            progress_color=THEME["danger"], button_color=THEME["danger"],
        ...
        ctk.CTkLabel(
            sleep_frame, text="Qualidade do Sono (1–5)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._sl_sleep = ctk.CTkSlider(
            sleep_frame, from_=1, to=5, number_of_steps=4,
            progress_color=THEME["success"], button_color=THEME["success"],
        ...
        self._txt_notes = ctk.CTkTextbox(
            scroll, font=themed_font("body"), height=80,
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"], corner_radius=RADIUS["input"],
        )
        self._txt_notes.pack(fill="x", pady=(0, 12))
        ctk.CTkLabel(
            scroll, text="Gatilhos (separados por vírgula)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._en_triggers = ctk.CTkEntry(
            scroll, placeholder_text="Ex: falta de sono, pressão acadêmica",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        ctk.CTkLabel(
            scroll, text="Atividades (separadas por vírgula)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._en_activities = ctk.CTkEntry(
            scroll, placeholder_text="Ex: estudou, fez exercício",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        ctk.CTkButton(
            ...
            width=110, height=38, corner_radius=RADIUS["button"],
            fg_color=THEME["divider"], hover_color=THEME["border"],
            text_color=THEME["text_muted"],
        ).pack(side="left", padx=(0, 8))
        PrimaryButton(
            footer, text=f"{ICONS['save']}  Salvar",
            command=self._salvar, height=38, width=140,
        ).pack(side="right")
...
class _CheckinModal(ctk.CTkToplevel):
    ...
    def _build(self, student_id, student_name):
        ...
        ctk.CTkLabel(
            type_frame, text="Tipo de Check-in",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._sel_type = ctk.CTkOptionMenu(
            type_frame, values=list(_CHECKIN_TYPE_LABELS.values()),
            fg_color=THEME["input_bg"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            height=40, corner_radius=RADIUS["input"],
            font=themed_font("body"),
        ...
        ctk.CTkLabel(
            wellbeing_frame, text="Bem-estar Geral (1–10)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._sl_wellbeing = ctk.CTkSlider(
            wellbeing_frame, from_=1, to=10, number_of_steps=9,
            progress_color=THEME["success"], button_color=THEME["success"],
            button_hover_color=THEME["success_strong"],
        )
        ...
        ctk.CTkLabel(
            scroll, text="Áreas de Atenção (separadas por vírgula)",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._en_areas = ctk.CTkEntry(
            scroll, placeholder_text="Ex: ansiedade, sono, estudos",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        ctk.CTkLabel(
            scroll, text="Recomendações",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        self._txt_recommendations = ctk.CTkTextbox(
            scroll, font=themed_font("body"), height=60,
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"], corner_radius=RADIUS["input"],
        )
        self._txt_recommendations.pack(fill="x", pady=(0, 12))
        ctk.CTkLabel(
            scroll, text="Notas do Profissional",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).pack(anchor="w", pady=(0, 6))
        ...
        ctk.CTkSwitch(
            follow_frame, text="Necessita acompanhamento",
            variable=self._var_followup, onvalue="on", offvalue="off",
            progress_color=THEME["primary"], button_color=THEME["surface"],
            button_hover_color=THEME["bg_alt"], fg_color=THEME["bg_alt"],
            corner_radius=RADIUS["pill"], cursor="hand2",
        ).pack(side="left")
        self._en_followup_date = ctk.CTkEntry(
            scroll, placeholder_text="Data do acompanhamento (YYYY-MM-DD)",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        PrimaryButton(
            footer, text=f"{ICONS['save']}  Salvar Check-in",
            command=self._salvar, height=38, width=160,
        ).pack(side="right")
...
class _ChallengeFormModal(ctk.CTkToplevel):
    ...
    def _build(self, data):
        ...
        scroll.pack(fill="both", expand=True, padx=SPACING["page_x"], pady=SPACING["page_y"])
        ctk.CTkLabel(
            scroll, text="Desafio de Bem-Estar",
            font=themed_font("h2", "bold"),
            text_color=THEME["text"],
        ).pack(anchor="w", pady=(0, 16))
        ctk.CTkLabel(
        ...
        self._en_title = ctk.CTkEntry(
            scroll, placeholder_text="Título do desafio",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        ctk.CTkLabel(
            row_cat, text="Categoria",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_secondary"],
        ).grid(row=0, column=0, sticky="w", pady=(0, 6))
        self._sel_cat = ctk.CTkOptionMenu(
            row_cat, values=[c[1] for c in _CHALLENGE_CATEGORIES],
            fg_color=THEME["input_bg"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            height=40, corner_radius=RADIUS["input"],
            font=themed_font("body"),
...
class BemEstarFrame(ctk.CTkScrollableFrame):
    def __init__(self, parent, controller):
        ...
        self._criar_visao_risco(content)
        self._criar_lista_checkins(content)
    ...
    def _criar_toolbar(self, content):
        ...
        ctk.CTkLabel(
            bar, text=f"{ICONS['mood_good']}  Bem-Estar",
            font=themed_font("h2", "bold"),
            text_color=THEME["text"],
        ).pack(side="left")
        ...
        self._sel_period = ctk.CTkOptionMenu(
            right, values=[o[0] for o in period_opts],
            variable=self._var_period,
            fg_color=THEME["input_bg"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            width=120, height=36, corner_radius=RADIUS["button"],
            font=themed_font("body"),
            command=lambda _: self._on_period_changed(),
        )
        self._sel_period.pack(side="left", padx=(0, 8))
        GhostButton(
            right, text=f"{ICONS['export']}  Exportar",
            command=self._exportar_dados, height=36, width=120,
        ).pack(side="left", padx=(0, 8))
        PrimaryButton(
            right, text=f"{ICONS['add']}  Registrar Humor",
            command=self._abrir_mood_modal, height=36, width=180,
        ).pack(side="left")
    ...
    def _abrir_mood_modal(self):
        ...
        self._mood_modal = _MoodEntryModal(
            self, on_save=self._salvar_mood_entry,
            student_id=sid, student_name=nome,
        )

    def _salvar_mood_entry(self, dados):
        def fetch():
            return self.controller_bem_estar.criar_entrada_humor(dados)
        def on_success(res):
            if res.get("success"):
    ...
    def _abrir_checkin_modal(self):
        ...
        self._checkin_modal = _CheckinModal(
            self, on_save=self._salvar_checkin,
            student_id=sid, student_name=nome,
        )

    def _salvar_checkin(self, dados):
        def fetch():
            return self.controller_bem_estar.criar_checkin(dados)
        def on_success(res):
            if res.get("success"):
    ...
    def _abrir_challenge_form(self, challenge_id=None):
        ...
        if challenge_id:
            def fetch():
                return self.controller_bem_estar.listar_desafios()
            def on_success(res):
                challenges = []
                if isinstance(res, dict):
                    challenges = (res.get("data") or {}).get("challenges") if isinstance(res.get("data"), dict) else res.get("data") or []
                ch = next((c for c in challenges if c.get("id") == challenge_id), {})
                self._challenge_modal = _ChallengeFormModal(
                    self, on_save=self._salvar_challenge,
                    challenge_id=challenge_id, challenge_data=ch,
                )
            def on_error(exc):
            ...
            return
        self._challenge_modal = _ChallengeFormModal(
            self, on_save=self._salvar_challenge, challenge_data=data,
        )

    def _salvar_challenge(self, dados, challenge_id=None):
        if challenge_id:
            def fetch():
                return self.controller_bem_estar.atualizar_desafio(challenge_id, dados)
            def on_success(res):
                if res.get("success"):
            ...
            def fetch():
                return self.controller_bem_estar.criar_desafio(dados)
            def on_success(res):
                if res.get("success"):
    ...
    def _deletar_desafio(self, challenge_id):
        if not self._confirmar("Excluir este desafio?"):
            return
        def fetch():
            return self.controller_bem_estar.deletar_desafio(challenge_id)
        def on_success(res):
            if res.get("success"):
    ...
    def _atribuir_desafio(self):
        ...
        def fetch():
            return self.controller_bem_estar.atribuir_desafio(dados)
        def on_success(res):
            if res.get("success"):
    ...
    def _completar_desafio(self, assignment_id):
        def fetch():
            return self.controller_bem_estar.completar_desafio(assignment_id)
        def on_success(res):
            if res.get("success"):
    ...
    def _desatribuir_desafio(self, assignment_id):
        if not self._confirmar("Remover atribuição deste desafio?"):
            return
        def fetch():
            return self.controller_bem_estar.desatribuir_desafio(assignment_id)
        def on_success(res):
            if res.get("success"):
    ...
    def _carregar_lista_estudantes(self):
        def fetch():
            return self.controller_bem_estar.listar_estudantes()
        def on_success(res):
            students = []
            if isinstance(res, dict):
                data = res.get("data")
    ...
    def _renderizar_lista_estudantes(self, lista: list):
        ...
        if not lista:
            EmptyState(
                self._student_list_body, icon=ICONS["mood_bad"],
    ...
    def _criar_item_estudante(self, st: dict):
        ...
        row.bind("<Enter>", lambda e, r=row, s=st: r.configure(fg_color=THEME["primary_soft"]) if self._selected_student != s else None)
        row.bind("<Leave>", lambda e, r=row, s=st: r.configure(fg_color=THEME["primary_soft"] if self._selected_student == s else THEME["bg_alt"]))
    ...
    def _carregar_perfil_estudante(self, student_id):
        def fetch():
            return self.controller_bem_estar.obter_humor_estudante(student_id)
        def on_success(res):
            ...
            if hasattr(self, "_profile_avatar"):
                for w in self._profile_avatar.winfo_children():
                    w.destroy()
                Avatar(self._profile_avatar, initials=student_name[:2], size=48, color=get_avatar_color(student_name)).pack(expand=True)
    ...
    def _draw_mini_chart(self, entries):
        ...
        for i in range(len(coords) - 1):
            canvas.create_line(coords[i][0], coords[i][1], coords[i+1][0], coords[i+1][1],
    ...
    def _carregar_historico_estudante(self, student_id):
        ...
        for w in self._history_body.winfo_children():
            w.destroy()
        def fetch():
            return self.controller_bem_estar.obter_historico_humor_estudante(student_id)
        def on_success(res):
            if not self.winfo_exists():
                return
            entries = []
            ...
            if not entries:
                EmptyState(
                    self._history_body, icon=ICONS["chart"],
    ...
    def _criar_row_historico(self, e: dict):
        ...
        inner.pack(fill="x", padx=spacing("md"), pady=spacing("sm"))
        inner.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(inner, text=emoji, font=themed_font("h3")).grid(row=0, column=0, padx=(0, spacing("md")))
        ctk.CTkLabel(inner, text=label, font=themed_font("body", "bold"), text_color=THEME["text"], anchor="w").grid(row=0, column=1, sticky="w")
        ctk.CTkLabel(inner, text=f"E:{energy} S:{stress}", font=themed_font("caption"), text_color=THEME["text_muted"], anchor="e").grid(row=0, column=2, sticky="e")
        ctk.CTkLabel(inner, text=str(date), font=themed_font("caption"), text_color=THEME["text_secondary"], anchor="w").grid(row=1, column=1, sticky="w", pady=(2, 0))
    ...
    def _carregar_checkins_estudante(self, student_id):
        ...
        for w in self._checkins_detail_body.winfo_children():
            w.destroy()
        def fetch():
            return self.controller_bem_estar.listar_checkins()
        def on_success(res):
            if not self.winfo_exists():
                return
            checkins = []
            if isinstance(res, dict):
                ...
                if isinstance(data, dict):
                    ...
                    checkins = data
            checkins = [c for c in checkins if c.get("student_id") == student_id or c.get("student_id") == str(student_id)]
            if not checkins:
                EmptyState(
                    self._checkins_detail_body, icon=ICONS["chart"],
    ...
    def _criar_row_checkin_detail(self, c: dict):
        ...
        ctk.CTkLabel(
            inner, text=f"Bem-estar: {wellbeing}/10" + ("  ! Acompanhamento" if followup else ""),
            font=themed_font("body"),
        ...
        if recommendations:
            ctk.CTkLabel(
                inner, text=str(recommendations)[:120],
                font=themed_font("caption"),
                text_color=THEME["text_secondary"], anchor="w",
            ).grid(row=2, column=0, sticky="w", pady=(2, 0))
        if areas:
            ctk.CTkLabel(
                inner, text="Áreas: " + ", ".join(str(a) for a in areas[:3]),
                font=themed_font("caption"),
                text_color=THEME["text_muted"], anchor="w",
            ).grid(row=3, column=0, sticky="w", pady=(2, 0))

    def _carregar_desafios_estudante(self, student_id):
        if not hasattr(self, "_challenges_detail_body"):
            return
        for w in self._challenges_detail_body.winfo_children():
            w.destroy()
        def fetch():
            return self.controller_bem_estar.listar_desafios_estudante(student_id)
        def on_success(res):
            if not self.winfo_exists():
                return
            assignments = []
            ...
            if not assignments:
                EmptyState(
                    self._challenges_detail_body, icon=ICONS["heart"],
    ...
    def _criar_row_challenge_detail(self, a: dict):
        ...
        if not is_completed:
            PrimaryButton(
                inner, text=f"{ICONS['check']}  Concluir",
                command=lambda aid=assignment_id: self._completar_desafio(aid),
                height=30, width=100, fg_color=THEME["success"],
                hover_color=THEME["success_strong"], text_color=THEME["text_on_primary"],
            ).grid(row=1, column=2, sticky="e", pady=(4, 0))
        DangerButton(
            inner, text=f"{ICONS['delete']}",
            command=lambda aid=assignment_id: self._desatribuir_desafio(aid),
            height=30, width=36, fg_color=THEME["danger"],
            hover_color=THEME["danger_strong"],
        ).grid(row=0, column=3, padx=(spacing("sm"), 0))

    def _carregar_dashboard_desafios(self):
        if not hasattr(self, "_dash_challenges_body"):
            return
        for w in self._dash_challenges_body.winfo_children():
            w.destroy()
        def fetch():
            return self.controller_bem_estar.obter_dashboard_desafios()
        def on_success(res):
            if not self.winfo_exists():
                return
            summary = {}
    ...
    def load_data(self):
        ...
        def on_error(exc):
            self._show_error(f"Não foi possível carregar os dados de bem-estar.\n{exc}", title="Erro de conexão")
    ...
    def _criar_kpis(self, content):
        ...
        kpis = [
            ("Humor Médio", f"{ICONS['mood_good']}  —", ICONS["heart"], THEME["kpi_blue"], THEME["kpi_blue_soft"], "_kpi_humor", "Média dos últimos 7 dias"),
            ("Participação", "—%", ICONS["chart"], THEME["kpi_pink"], THEME["kpi_pink_soft"], "_kpi_part", "Taxa de check-ins"),
            ("Alertas Críticos", "—", ICONS["alert"], THEME["kpi_red"], THEME["kpi_red_soft"], "_kpi_crit", "Estudantes em situação crítica"),
        ]
        for i, (title, initial, icon, accent, soft, attr, sub) in enumerate(kpis):
            row.grid_columnconfigure(i, weight=1)
            card = KPICard(row, title=title, value=initial, icon=icon, accent=accent, unit="", size="md")
            card.grid(row=0, column=i, sticky="ew", padx=SPACING["grid_gap"] // 2)
            setattr(self, attr, card)

    def _criar_secao_grafico(self, content):
        outer, body = _section_card(content, f"{ICONS['chart']}  Tendência de Bem-Estar — últimos {self._period_days} dias")
        ...
        self.canvas_30d = ctk.CTkCanvas(chart_wrap, bg=THEME["surface"], height=200, highlightthickness=0)
        ...
        self._chart_empty = EmptyState(chart_wrap, icon=ICONS["chart"], title="Sem dados de humor", subtitle="Os registros aparecerão aqui quando houver entradas")
        ...
        for label, color, pct_key, default in [
            ...
            ctk.CTkLabel(col, text=label, font=themed_font("body", "bold"), text_color=THEME["text_secondary"]).pack(anchor="w", pady=(0, spacing("xs")))
            bar_bg = ctk.CTkFrame(col, height=8, fg_color=THEME["chart_grid"], corner_radius=RADIUS["pill"])
            ...
            pct_lbl = ctk.CTkLabel(col, text=f"{default}%", font=themed_font("body", "bold"), text_color=THEME["text"])
    ...
    def _draw_chart(self, data=None):
        ...
        if cw < 80 or ch < 60:
            return
        pts = [d.get("mood_level") or d.get("avg_mood") or d.get("media_humor") or 3.0 for d in self._chart_data] if self._chart_data else []
        if not pts:
            self.canvas_30d.delete("all")
        ...
        for i in range(6):
            ...
            self.canvas_30d.create_text(mx - 6, gy, text=str(v), font=(FONT_FAMILY, 8), fill=THEME["text_muted"], anchor="e")
        coords = [(mx + i * cw2 / max(n - 1, 1), (ch - my) - ((v - 1) * ch2 / 4)) for i, v in enumerate(pts)]
        poly = []
        for x, y in coords:
            poly += [x, y]
        ...
        for i in range(len(coords) - 1):
            self.canvas_30d.create_line(coords[i][0], coords[i][1], coords[i+1][0], coords[i+1][1], fill=THEME["chart_line"], width=2.5, capstyle="round", joinstyle="round")
        for i, (x, y) in enumerate(coords):
            v = pts[i]
            dot_c = THEME["dot_bad"] if v < 2.5 else (THEME["dot_mid"] if v < 3.5 else THEME["dot_good"])
            self.canvas_30d.create_oval(x-4, y-4, x+4, y+4, fill=dot_c, outline=THEME["surface"], width=2)
        step = max(1, n // 7)
        for i, (x, _) in enumerate(coords):
            if i % step == 0 and self._chart_data:
                ...
                if len(lbl) > 5:
                    lbl = lbl[5:]
                self.canvas_30d.create_text(x, ch - 6, text=lbl, font=(FONT_FAMILY, 8), fill=THEME["text_secondary"])
    ...
    def _criar_area_estudante(self, content):
        ...
        hdr.pack(fill="x", padx=SPACING["card_pad"], pady=(SPACING["card_pad"], SPACING["label_gap"]))
        ctk.CTkLabel(
            hdr, text=f"{ICONS['user']}  Visão por Estudante",
        ...
        self._student_search = ctk.CTkEntry(
            search_row, placeholder_text="Buscar estudante por nome...",
            fg_color=THEME["input_bg"], border_width=1,
            border_color=THEME["input_border"],
            font=themed_font("body"), height=40,
        )
        ...
        self._student_list_body = ctk.CTkScrollableFrame(
            body, fg_color="transparent", height=160,
            scrollbar_button_color=THEME["border_strong"],
            scrollbar_button_hover_color=THEME["text_muted"],
        )
        ...
        profile_hdr = ctk.CTkFrame(detail_card, fg_color="transparent")
        profile_hdr.grid(row=0, column=0, sticky="ew", padx=SPACING["card_pad"], pady=(SPACING["card_pad"], 0))
        self._profile_avatar = ctk.CTkFrame(profile_hdr, width=48, height=48, fg_color="transparent")
        self._profile_avatar.pack(side="left", padx=(0, spacing("md")))
        self._profile_avatar.pack_propagate(False)
        Avatar(self._profile_avatar, initials="??", size=48, color=THEME["primary"]).pack(expand=True)
        ...
        self._profile_name = ctk.CTkLabel(
            name_stack, text="Selecione um estudante",
            font=themed_font("h4", "bold"),
            text_color=THEME["text"], anchor="w",
        )
        self._profile_name.pack(anchor="w")
        self._profile_course = ctk.CTkLabel(
            name_stack, text="Busque ou selecione na lista",
            font=themed_font("caption"),
            text_color=THEME["text_secondary"], anchor="w",
        )
        ...
        PrimaryButton(actions, text=f"{ICONS['mood_good']}  Humor", command=self._abrir_mood_modal, height=36, width=110).pack(side="left", padx=(0, spacing("xs")))
        PrimaryButton(actions, text=f"{ICONS['chart']}  Check-in", command=self._abrir_checkin_modal, height=36, width=110, fg_color=THEME["info"], hover_color=THEME["info_strong"], text_color=THEME["text_on_primary"]).pack(side="left", padx=(0, spacing("xs")))
        mini_chart_card = Card(detail_card, auto_body=False)
        mini_chart_card.grid(row=1, column=0, sticky="nsew", padx=SPACING["card_pad"], pady=(0, SPACING["card_pad"]))
        mini_chart_card.grid_rowconfigure(0, weight=1)
        mini_chart_card.grid_columnconfigure(0, weight=1)
        self._mini_chart_canvas = ctk.CTkCanvas(mini_chart_card, bg=THEME["surface"], height=80, highlightthickness=0)
        ...
        self._detail_tabs.grid(row=2, column=0, sticky="nsew", padx=SPACING["card_pad"], pady=(0, SPACING["card_pad"]))
        ...
        self._history_body = ctk.CTkScrollableFrame(self._tab_history, fg_color="transparent", scrollbar_button_color=THEME["border_strong"], scrollbar_button_hover_color=THEME["text_muted"])
        self._history_body.pack(fill="both", expand=True)
        self._checkins_detail_body = ctk.CTkScrollableFrame(self._tab_checkins, fg_color="transparent", scrollbar_button_color=THEME["border_strong"], scrollbar_button_hover_color=THEME["text_muted"])
        self._checkins_detail_body.pack(fill="both", expand=True)
        self._challenges_detail_body = ctk.CTkScrollableFrame(self._tab_challenges, fg_color="transparent", scrollbar_button_color=THEME["border_strong"], scrollbar_button_hover_color=THEME["text_muted"])
    ...
    def _filtrar_lista_estudantes(self, _=None):
        ...
        filtrados = [s for s in self._all_students if termo in s.get("name", "").lower()] if termo else self._all_students
    ...
    def _criar_visao_risco(self, content):
        ...
        hdr.pack(fill="x", padx=SPACING["card_pad"], pady=(SPACING["card_pad"], SPACING["label_gap"]))
        ctk.CTkLabel(hdr, text=f"{ICONS['mood_good']}  Visão de Risco dos Estudantes", font=themed_font("h4", "bold"), text_color=THEME["text"]).pack(side="left")
        ctk.CTkLabel(hdr, text="Classificação por nível de atenção necessária", font=themed_font("body_sm"), text_color=THEME["text_secondary"]).pack(side="left", padx=(10, 0), pady=(2, 0))
        ...
        for i, (title, color, soft, key) in enumerate(_RISK_COLS):
            col_card = ctk.CTkFrame(cols_wrap, fg_color=THEME["surface"], corner_radius=RADIUS["card"], border_width=1, border_color=THEME["border"])
            ...
            ctk.CTkFrame(col_hdr, width=10, height=10, corner_radius=5, fg_color=color).grid(row=0, column=0, padx=(spacing("xs"), spacing("sm")))
            ctk.CTkLabel(col_hdr, text=title, font=themed_font("body", "bold"), text_color=color).grid(row=0, column=1, sticky="w")
            count_lbl = ctk.CTkLabel(col_hdr, text="0", font=themed_font("body", "bold"), text_color=color)
            count_lbl.grid(row=0, column=2, padx=(spacing("sm"), spacing("xs")))
            body = ctk.CTkScrollableFrame(col_card, fg_color="transparent", height=220, scrollbar_button_color=THEME["border_strong"], scrollbar_button_hover_color=THEME["text_muted"])
            body.grid(row=1, column=0, sticky="nsew", padx=spacing("sm"), pady=(0, spacing("sm")))
            self.colunas_risco[key] = {"content": body, "count_lbl": count_lbl, "color": color, "soft": soft}
    ...
    def populate_risks(self, risks: list):
        ...
        if not risks:
            for key in self.colunas_risco:
                EmptyState(self.colunas_risco[key]["content"], icon=ICONS["mood_bad"], title="Nenhum estudante", subtitle="").pack(pady=4)
            return
        ...
        for s in risks:
            ...
            if nivel not in self.colunas_risco:
                nivel = "normal"
            counts[nivel] += 1
            batch.add(lambda s=s, n=nivel: self._criar_card_risco(self.colunas_risco[n]["content"], s, self.colunas_risco[n]["color"], self.colunas_risco[n]["soft"]))
        batch.execute()
    ...
    def _criar_card_risco(self, parent, student: dict, color: str, soft: str):
        ...
        card = ctk.CTkFrame(parent, fg_color=THEME["surface"], corner_radius=RADIUS["lg"], border_width=1, border_color=THEME["border"])
        ...
        av.grid(row=0, column=0, rowspan=2, padx=(0, spacing("md")), sticky="ns")
        ctk.CTkLabel(inner, text=nome, font=themed_font("body", "bold"), text_color=THEME["text"], anchor="w").grid(row=0, column=1, sticky="w")
        ctk.CTkLabel(inner, text=curso, font=themed_font("caption"), text_color=THEME["text_secondary"], anchor="w").grid(row=1, column=1, sticky="w")
        if msg:
            chip = ctk.CTkFrame(card, fg_color=soft, corner_radius=RADIUS["sm"])
            chip.pack(fill="x", padx=spacing("lg"), pady=(0, spacing("md")))
            ctk.CTkLabel(chip, text=msg, font=themed_font("caption", "bold"), text_color=color, wraplength=140, anchor="w").pack(padx=spacing("sm"), pady=spacing("xs"), anchor="w")
    ...
    def populate_checkins(self, checkins: list):
        ...
        if not isinstance(checkins, list):
            checkins = []
        if not checkins:
            EmptyState(self._checkins_body, icon=ICONS["chart"], title="Nenhum check-in registrado", subtitle="Os check-ins aparecerão aqui quando forem realizados").pack(pady=8)
            return
        batch = WidgetBatchBuilder(parent=self, batch_size=20)
        for c in checkins:
    ...
    def _criar_row_checkin(self, c: dict):
        ...
        row = ctk.CTkFrame(self._checkins_body, fg_color=THEME["bg_alt"], corner_radius=RADIUS["lg"])
        ...
        chip_bg = ctk.CTkFrame(mood_frame, fg_color=blend_color(color, 0.15), corner_radius=RADIUS["button"])
        chip_bg.pack(anchor="e")
        ctk.CTkLabel(chip_bg, text=f"{emoji}  {texto}", font=themed_font("body", "bold"), text_color=color).pack(padx=spacing("sm"), pady=spacing("xs"))
        ctk.CTkLabel(mood_frame, text=data, font=themed_font("caption"), text_color=THEME["text_muted"]).pack(anchor="e", pady=(spacing("xs"), 0))
