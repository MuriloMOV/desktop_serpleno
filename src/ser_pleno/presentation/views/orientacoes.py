...
class OrientationHistoryCard(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # ── Topo: círculo de data + info + açÓµes ─────────────────────────────
        ...
        for label, cmd, accent, s in [
            ("ðŸ‘  Ver",      lambda: self._on_view(self._o),
             O["accent_soft"], O["accent"]),
            ("âœ  Editar",   lambda: self._on_edit(self._o),
             O["accent"],     "#FFFFFF"),
            ("â§‰  Duplicar", lambda: self._on_duplicate(self._o.get("id")),
             O["divider"],    O["text_muted"]),
            ("ðŸ—‘  Excluir",  lambda: self._on_delete(self._o.get("id")),
...
class OrientacoesFrame(ctk.CTkScrollableFrame):
    ...
    def _criar_cabecalho(self):
        ...
        left.pack(side="left")
        ctk.CTkLabel(left, text="OrientaçÓµes",
    ...
    def _criar_sidebar_estudantes(self, parent):
        ...
        ctk.CTkLabel(search_wrap, text="ðŸ”",
                     font=font(size=13),
    ...
    def _criar_painel_principal(self, parent):
        ...
        for key, label in [("historico", "ðŸ“‹  Histórico"),
                           ("nova",      "ï¼‹  Nova Orientação")]:
            btn = ctk.CTkButton(
        ...
        self._hist_placeholder = ctk.CTkLabel(
            ...
            text="Selecione um estudante para ver as orientaçÓµes",
            font=font(size=13),
            text_color=O["text_muted"],
        )
    ...
    def _construir_form_nova(self, parent):
        ...
        self.f_titulo = FormField(body, "ðŸ“  Título", placeholder="Título da orientação")
        self.f_titulo.pack(fill="x", pady=(0, 10))

        self.f_conteudo = FormField(body, "ðŸ“„  Conteúdo",
        ...
        ctk.CTkButton(footer, text="âœ”  Salvar Orientação",
    ...
    def _carregar_dados(self):
        ...
        def on_error(exc):
            logger.error("Erro ao carregar orientaçÓµes: %s", exc)
    ...
    def _renderizar(self, resultado):
        ...
        if not orientacoes:
            ctk.CTkLabel(self._area_historico,
                         text="ðŸ“‹  Nenhuma orientação registrada",
                         font=font(size=13),
    ...
    def _popular_sidebar(self, estudantes: list):
        ...
        ctk.CTkLabel(todos_row, text="ðŸ“‹  Todos os estudantes",
                     font=font(size=12, weight="bold"),
    ...
    def _mostrar_orientacoes(self, orientacoes: list):
        ...
        if not orientacoes:
            ctk.CTkLabel(self._area_historico,
                         text="ðŸ“‹  Nenhuma orientação para este estudante",
                         font=font(size=13),
                         text_color=O["text_muted"]).pack(pady=30)
            return
    ...
    def _modal_detalhe(self, o: dict):
        ...
        for label, value in [
            ("ðŸ“…  Data",          (o.get("session_date") or "—")[:10]),
            ("ðŸ·  Tema",          tema),
            ("ðŸ”—  Encaminhamento", o.get("referral") or "—"),
        ]:
