...
from ser_pleno.ui.theme import (
    THEME, SPACING, RADIUS, font, themed_font, blend_color
)
...
from ser_pleno.presentation.components.ui_components import (
    Card, PrimaryButton, GhostButton, DangerButton, Badge, EmptyState,
    Divider, bind_clickable, KPICard, SkeletonLoader, Tooltip, Toast, BaseModal,
)
...
def _category_color(category: str) -> tuple:
    from ser_pleno.application.services.metas import CATEGORY_COLORS
...
class GoalCard(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        for label, cmd, bg, txt in [
            (f"{ICONS['view']} Ver", lambda: self._on_view(self._goal), THEME["primary_soft"], THEME["primary"]),
            (f"{ICONS['edit']} Editar", lambda: self._on_edit(self._goal), THEME["accent_soft"], THEME["accent"]),
            (f"{ICONS['delete']} Excluir", lambda: self._on_delete(self._goal.get("id")), THEME["danger_soft"], THEME["danger"]),
        ]:
            ctk.CTkButton(
                acts, text=label, command=cmd,
                height=28, width=80, corner_radius=8,
                fg_color=bg, hover_color=bg,
                text_color=txt, font=font(size=11, weight="bold"),
        ...
        target_txt = f"{ICONS['calendar']} Prazo: {target_date}" if target_date else f"{ICONS['calendar']} Sem prazo"
        ctk.CTkLabel(
            meta_frame, text=target_txt,
            font=font(size=11),
            text_color=THEME["text_muted"],
        ).pack(side="left")

        ctk.CTkLabel(
            meta_frame, text=f"{ICONS['user']} {student_name}",
            font=font(size=11),
            text_color=THEME["text_muted"],
        ).pack(side="right")
...
class MetasFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.controller_metas = controller
    ...
    def _criar_filtros(self):
        ...
        PrimaryButton(
            right, text=f"{ICONS['add']} Nova Meta",
        ...
        self.f_estudante = ctk.CTkOptionMenu(
            right, values=["Todos os estudantes"],
        ...
        self.f_status = ctk.CTkOptionMenu(
            right, values=["Todos", "Nao iniciada", "Em andamento", "Concluida", "Pausada", "Cancelada"],
            command=lambda _: self._aplicar_filtros(),
            **opt_style,
        )
        ...
        self.f_prioridade = ctk.CTkOptionMenu(
            right, values=["Todas", "Baixa", "Media", "Alta", "Urgente"],
            command=lambda _: self._aplicar_filtros(),
            **opt_style,
        )
        ...
        self.f_categoria = ctk.CTkOptionMenu(
            right, values=["Todas"] + [
                "Academico", "Emocional", "Social", "Familiar", "Vocacional", "Comportamental", "Geral"
            ],
            command=lambda _: self._aplicar_filtros(),
            **opt_style,
        )
        ...
        # Busca
        search_wrap = ctk.CTkFrame(filter_frame, fg_color=THEME["bg_alt"], corner_radius=RADIUS["input"])
        search_wrap.pack(side="left", fill="x", expand=True, padx=(0, 12))

        ctk.CTkLabel(
            search_wrap, text=ICONS["search"],
            font=font(size=13), text_color=THEME["text_muted"],
        ...
        self.entry_busca = ctk.CTkEntry(
            ...
            font=themed_font("body"), height=32,
        )
    ...
    def _criar_lista(self):
        ...
        self.scroll_list.grid(row=2, column=0, sticky="nsew", padx=SPACING["page_x"], pady=(0, SPACING["page_y"]))
    ...
    def _carregar_estatisticas(self):
        def fetch():
            return self.controller_metas.obter_estatisticas()
    ...
    def _carregar_metas(self):
        def fetch():
            return self.controller_metas.listar_metas()
    ...
    def _mostrar_skeletons(self):
        ...
        for _ in range(6):
            batch.add(lambda: SkeletonLoader(self.scroll_list, width=260, height=80, variant="card").pack(
    ...
    def _mostrar_metas(self, metas: list):
        ...
        if not metas:
            EmptyState(
                self.scroll_list, icon=ICONS["chart"], title="Nenhuma meta registrada",
        ...
        batch = WidgetBatchBuilder(parent=self.scroll_list, batch_size=20)
        for m in metas:
            batch.add(lambda m=m: GoalCard(
                self.scroll_list, goal=m,
    ...
    def _carregar_estudantes_filtro(self):
        def fetch():
            return self.controller_metas.listar_estudantes()
    ...
    def _aplicar_filtros(self):
        ...
        estudante_raw = self.f_estudante.get() if hasattr(self, "f_estudante") else "Todos os estudantes"
        ...
        status_map = {
            "Todos": "", "Nao iniciada": "not_started", "Em andamento": "in_progress",
            "Concluida": "completed", "Pausada": "paused", "Cancelada": "cancelled",
        }
        priority_map = {
            "Todas": "", "Baixa": "low", "Media": "medium", "Alta": "high", "Urgente": "urgent",
        }
        category_map = {
            "Todas": "", "Academico": "Academico", "Emocional": "Emocional",
            "Social": "Social", "Familiar": "Familiar", "Vocacional": "Vocacional",
            "Comportamental": "Comportamental", "Geral": "Geral",
        }
    ...
    def _excluir_meta(self, meta_id):
        ...
        ctk.CTkLabel(
            confirm, text="Esta acao nao pode ser desfeita.",
        ...
        ctk.CTkButton(
            btn_frame, text="Cancelar", command=confirm.destroy,
        ...
        def confirmar():
            ...
            def delete():
                return self.controller_metas.deletar_meta(meta_id)
        ...
        ctk.CTkButton(
            btn_frame, text="Excluir", command=confirmar,
            height=36, corner_radius=10, width=120,
            fg_color=THEME["danger"], hover_color=THEME["danger_hover"],
            text_color="white", font=themed_font("button", "bold"),
    ...
    def _modal_meta(self, meta=None):
        ...
        ctk.CTkLabel(
            card, text="Editar Meta" if is_edit else "Nova Meta",
        ...
        estudantes = []
        try:
            resp = self.controller_metas.listar_estudantes()
            if resp.get("success"):
        ...
        f_estudante = ctk.CTkOptionMenu(
            card, values=student_values if student_values else ["Nenhum estudante"],
            fg_color=THEME["bg_alt"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
        ...
        f_titulo = ctk.CTkEntry(
            card, placeholder_text="Título da meta",
            fg_color=THEME["bg_alt"], border_width=1, border_color=THEME["border"],
            text_color=THEME["text"], font=themed_font("body"), height=40,
        )
        f_titulo.pack(fill="x", pady=(0, 10))

        f_descricao = ctk.CTkTextbox(
            card, height=80, fg_color=THEME["bg_alt"], border_width=1, border_color=THEME["border"],
            font=themed_font("body"), corner_radius=RADIUS["input"],
        )
        ...
        f_categoria = ctk.CTkOptionMenu(
            row, values=[
                "Academico", "Emocional", "Social", "Familiar",
                "Vocacional", "Comportamental", "Geral",
            ],
            fg_color=THEME["bg_alt"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            dropdown_fg_color=THEME["surface"], dropdown_text_color=THEME["text"],
        ...
        f_prioridade = ctk.CTkOptionMenu(
            row, values=["low", "medium", "high", "urgent"],
            fg_color=THEME["bg_alt"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            dropdown_fg_color=THEME["surface"], dropdown_text_color=THEME["text"],
            font=themed_font("body"), height=36, corner_radius=RADIUS["input"],
        ...
        f_status = ctk.CTkOptionMenu(
            row2, values=["not_started", "in_progress", "completed", "paused", "cancelled"],
            fg_color=THEME["bg_alt"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            dropdown_fg_color=THEME["surface"], dropdown_text_color=THEME["text"],
            font=themed_font("body"), height=36, corner_radius=RADIUS["input"],
        ...
        f_prazo = ctk.CTkEntry(
            row2, placeholder_text="Prazo (YYYY-MM-DD)",
            fg_color=THEME["bg_alt"], border_width=1, border_color=THEME["border"],
            text_color=THEME["text"], font=themed_font("body"), height=36,
        )
        ...
        f_notas = ctk.CTkTextbox(
            card, height=60, fg_color=THEME["bg_alt"], border_width=1, border_color=THEME["border"],
            font=themed_font("body"), corner_radius=RADIUS["input"],
        )
        ...
        def salvar():
            ...
            estudante_idx = student_values.index(f_estudante.get()) if f_estudante.get() in student_values else 0
            ...
            def save():
                if is_edit:
                    return self.controller_metas.atualizar_meta(meta.get("id"), dados)
                return self.controller_metas.criar_meta(dados)
        ...
        ctk.CTkButton(
            footer, text=f"{ICONS['check']} Salvar",
            command=salvar,
            height=36, width=150, corner_radius=10,
    ...
    def _modal_detalhe(self, meta):
        ...
        ctk.CTkLabel(ib, text=category[0].upper(),
        ...
        fields = [
            ...
            (f"{ICONS['status_dot']} Status", (meta.get("status", "—") or "—").replace("_", " ").title()),
        ...
        for i, (label, value) in enumerate(fields):
            ...
            box.grid(row=r, column=c, padx=(0 if c == 0 else 6, 6 if c == 0 else 0), pady=4, sticky="nsew")
            ctk.CTkLabel(
                box, text=label,
        ...
        if meta.get("description"):
            ctk.CTkLabel(
                scroll, text="Descricao",
                font=themed_font("caption", "bold"),
                text_color=THEME["text_muted"],
            ).pack(anchor="w", pady=(0, 4))
            ctk.CTkLabel(
                scroll, text=meta.get("description", ""),
                font=themed_font("body"),
                text_color=THEME["text"],
        ...
        if meta.get("success_criteria"):
            ctk.CTkLabel(
                scroll, text="Criterios de Sucesso",
                font=themed_font("caption", "bold"),
                text_color=THEME["text_muted"],
            ).pack(anchor="w", pady=(0, 4))
            ctk.CTkLabel(
                scroll, text=meta.get("success_criteria", ""),
                font=themed_font("body"),
                text_color=THEME["text"],
                wraplength=540, justify="left", anchor="w",
            ).pack(anchor="w", pady=(0, 12))
        ...
        ctk.CTkLabel(
            scroll, text="Historico de Progresso",
            font=themed_font("caption", "bold"),
            text_color=THEME["text_muted"],
        ).pack(anchor="w", pady=(0, 4))

        self._progresso_list = ctk.CTkScrollableFrame(
            scroll, fg_color="transparent", height=200,
            scrollbar_button_color=THEME["border_strong"],
        )
        ...
        def carregar_progresso():
            def fetch():
                return self.controller_metas.listar_progresso(meta.get("id"))
        ...
        ctk.CTkButton(
            btn_frame, text=f"{ICONS['edit']} Editar",
            command=lambda: (modal.destroy(), self._editar_meta(meta)),
            height=36, width=120, corner_radius=10,
            fg_color=THEME["primary_soft"], hover_color=THEME["primary"],
            text_color=THEME["primary"],
        ).pack(side="left", padx=(0, 8))

        ctk.CTkButton(
            btn_frame, text=f"{ICONS['clock']} Registrar Progresso",
            command=lambda: (modal.destroy(), self._abrir_modal_progresso(meta)),
            height=36, width=180, corner_radius=10,
            fg_color=THEME["accent_soft"], hover_color=THEME["accent"],
            text_color=THEME["accent"],
        ).pack(side="left", padx=(0, 8))

        ctk.CTkButton(
            btn_frame, text="Fechar", command=modal.destroy,
    ...
    def _renderizar_progresso(self, result):
        ...
        if not progressos:
            ctk.CTkLabel(
                self._progresso_list, text="Nenhum progresso registrado",
                font=themed_font("body"),
                text_color=THEME["text_muted"],
            ).pack(pady=20)
            return
        ...
        for p in progressos:
            row = ctk.CTkFrame(self._progresso_list, fg_color=THEME["bg_alt"], corner_radius=RADIUS["lg"])
            row.pack(fill="x", pady=4)

            ctk.CTkLabel(
                row, text=f"{p.get('percentage', 0)}%",
                font=font(size=14, weight="bold"),
                text_color=THEME["primary"],
            ...
            ctk.CTkLabel(
                info, text=p.get("notes", "") or "Sem notas",
                font=themed_font("body"),
                text_color=THEME["text"],
                anchor="w",
            ).pack(anchor="w")
    ...
    def _abrir_modal_progresso(self, meta):
        ...
        ctk.CTkLabel(
            card, text=f"Progresso: {meta.get('title', 'Meta')}",
            font=themed_font("h4", "bold"),
            text_color=THEME["text"],
        ).pack(anchor="w", pady=(0, 16))
        ...
        ctk.CTkLabel(
            card, text=f"Progresso atual: {current}%",
            font=themed_font("body"),
            text_color=THEME["text_muted"],
        ...
        def salvar():
            ...
            def save():
                return self.controller_metas.registrar_progresso(
                    meta.get("id"), percentage, notes
                )
        ...
        ctk.CTkButton(
            footer, text=f"{ICONS['check']} Registrar",
            command=salvar,
            height=36, width=150, corner_radius=10,
            fg_color=THEME["primary"], hover_color=THEME["primary_hover"],
            text_color="white", font=themed_font("button", "bold"),
