...
from ser_pleno.presentation.components.ui_components import Divider, EmptyState, PrimaryButton, GhostButton, DangerButton, Card, BaseModal, Toast
...
_QUICK_CARDS = [
    (ICONS["document"], "Relatório Geral",       "geral",       "Visão completa do sistema",         THEME["kpi_blue"],   THEME["kpi_blue_soft"]),
    (ICONS["calendar"], "Agendamentos",           "agendamentos","Análise de consultas",              THEME["kpi_green"],  THEME["kpi_green_soft"]),
    (ICONS["user"],     "Intervenções",           "intervencoes","Acompanhamentos realizados",        THEME["kpi_violet"], THEME["kpi_violet_soft"]),
    (ICONS["search"],   "Triagens",               "triagens",    "Análise de triagens",               THEME["kpi_amber"],  THEME["kpi_amber_soft"]),
]
...
class RelatorioFrame(BaseViewFrame):
    ...
    def _carregar_dados(self):
        ...
        def _on_error(exc):
            self._show_error(f"Não foi possível carregar relatórios.\n{exc}")
    ...
    def _atualizar_view(self, stats_res, reports_res):
        if stats_res.get("success"):
            ...
            mapping_resumo = {
                ...
                "consultas_30d":      summary.get("appointments_30d", summary.get("appointments_total", "—")),
                "intervencoes_30d":   summary.get("interventions_30d", summary.get("interventions_total", "—")),
    ...
    def _criar_cards_relatorios_rapidos(self):
        ...
        for idx, (icon, title, rtype, subtitle, accent, soft) in enumerate(_QUICK_CARDS):
            ...
            wrap.grid(row=0, column=idx, sticky="ew", padx=(0 if idx == 0 else SPACING["icon_gap"] // 2))
    ...
    def _criar_card_grafico(self, parent):
        ...
        body.pack(fill="both", expand=True, padx=SPACING["card_pad"], pady=(0, SPACING["label_gap"]))
        ...
        self._comparison_btn = ctk.CTkButton(
            hdr, text=f"{ICONS['layout']} Comparar períodos",
            command=self._toggle_comparacao,
            width=160, height=32,
            fg_color="transparent", border_width=1, border_color=THEME["border"],
            text_color=THEME["text_secondary"], hover_color=THEME["bg_alt"],
            font=themed_font("body_sm", "bold"), corner_radius=RADIUS["button"],
        )
        ...
        self._comparison_panel = ctk.CTkFrame(body, fg_color=THEME["bg_alt"], corner_radius=RADIUS["md"])
        ...
        for label, key in [("Período A", "a"), ("Período B", "b")]:
            ...
            col.pack(side="left", expand=True, fill="x", padx=(0 if key == "a" else SPACING["icon_gap"], 0))
            ctk.CTkLabel(col, text=label, font=themed_font("body_sm", "bold"),
                         text_color=THEME["text_secondary"]).pack(anchor="w")
            row = ctk.CTkFrame(col, fg_color="transparent")
            row.pack(fill="x", pady=(SPACING["xs"], 0))
            setattr(self, f"_comp_{key}_start", ctk.CTkEntry(row, width=120, height=32,
                placeholder_text="Início", font=themed_font("body_sm")))
            getattr(self, f"_comp_{key}_start").pack(side="left", padx=(0, SPACING["xs"]))
            setattr(self, f"_comp_{key}_end", ctk.CTkEntry(row, width=120, height=32,
                placeholder_text="Fim", font=themed_font("body_sm")))
            getattr(self, f"_comp_{key}_end").pack(side="left")
        ...
        PrimaryButton(comp_btns, text="Aplicar", command=self._aplicar_comparacao,
                      width=100, height=32, size="sm").pack(side="right", padx=(SPACING["xs"], 0))
        GhostButton(comp_btns, text="Limpar", command=self._limpar_comparacao,
        ...
        self._chart_empty.grid(row=0, column=0, sticky="nsew", padx=SPACING["icon_gap"], pady=SPACING["icon_gap"])
    ...
    def _aplicar_comparacao(self):
        ...
        a_end = getattr(self, "_comp_b_end", None)
        ...
        try:
            ...
            res = self.controller_relatorio.obter_comparacao_estatisticas(periodo_inicio, periodo_fim)
            if res.get("success"):
                data = res.get("data", {})
                ...
                lines = []
                lines.append(f"Período A: {periodo_inicio[0]} a {periodo_inicio[1]}")
                lines.append(f"  Consultas: {period_a.get('appointments_total', 0)}")
                lines.append(f"  Intervenções: {period_a.get('interventions_total', 0)}")
                lines.append(f"  Triagens: {period_a.get('screenings_total', 0)}")
                lines.append(f"  Taxa comparecimento: {period_a.get('attendance_rate', 0)}%")
                lines.append("")
                lines.append(f"Período B: {periodo_fim[0]} a {periodo_fim[1]}")
                lines.append(f"  Consultas: {period_b.get('appointments_total', 0)}")
                lines.append(f"  Intervenções: {period_b.get('interventions_total', 0)}")
                lines.append(f"  Triagens: {period_b.get('screenings_total', 0)}")
                lines.append(f"  Taxa comparecimento: {period_b.get('attendance_rate', 0)}%")
                lines.append("")
    ...
    def _criar_lista_relatorios(self):
        ...
        self.filtro_busca = ctk.CTkEntry(
            filters, placeholder_text="Buscar...", width=160, height=34,
            font=themed_font("body_sm"), corner_radius=RADIUS["input"],
        )
        ...
        self.filtro_data_inicio = ctk.CTkEntry(
            filters, placeholder_text="De", width=100, height=34,
            font=themed_font("body_sm"), corner_radius=RADIUS["input"],
        )
        ...
        self.filtro_data_fim = ctk.CTkEntry(
            filters, placeholder_text="Até", width=100, height=34,
            font=themed_font("body_sm"), corner_radius=RADIUS["input"],
        )
        ...
        PrimaryButton(filters, text="Filtrar", command=self._filtrar_relatorios,
                      width=80, height=34, size="sm").pack(side="left")

        Divider(card.body).pack(fill="x", pady=(SPACING["label_gap"], 0))

        self._bulk_bar = ctk.CTkFrame(card.body, fg_color=THEME["primary_soft"], corner_radius=RADIUS["md"])
        ...
        self._bulk_count_lbl = ctk.CTkLabel(
            bulk_inner, text="0 selecionado(s)",
            font=themed_font("body", "bold"), text_color=THEME["primary"],
        )
        self._bulk_count_lbl.pack(side="left")

        PrimaryButton(bulk_inner, text=f"{ICONS['download']} Baixar ZIP",
                      command=self._bulk_download, width=120, height=34, size="sm").pack(side="right", padx=(SPACING["xs"], 0))
        DangerButton(bulk_inner, text=f"{ICONS['delete']} Excluir",
                     command=self._bulk_delete, width=100, height=34).pack(side="right", padx=(0, SPACING["xs"]))
        GhostButton(bulk_inner, text="Cancelar", command=self._limpar_selecao,
                    width=80, height=34).pack(side="right")
        ...
        ctk.CTkCheckBox(
            col_hdr, text="", variable=self._select_all_var,
    ...
    def _filtrar_relatorios(self):
        ...
        def fetch():
            return self.controller_relatorio.listar_relatorios(
                tipo=tipo_map.get(tipo), search=search, data_inicio=data_inicio, data_fim=data_fim
            )

        def _on_error(exc):
            self._show_error(f"Falha ao filtrar.\n{exc}")

        AsyncRunner.run(
            task=fetch,
            on_success=lambda res: self._on_filter_success(res),
            on_error=_on_error,
            widget_ref=self,
        )
    ...
    def _bulk_delete(self):
        ...
        if not self._confirmar(f"Excluir {len(self._selecionados)} relatório(s)?"):
            return
        try:
            res = self.controller_relatorio.deletar_lote(list(self._selecionados))
            if res.get("success"):
    ...
    def _bulk_download(self):
        if not self._selecionados:
            return
        try:
            res = self.controller_relatorio.baixar_lote(list(self._selecionados))
            if res.get("success"):
    ...
    def _criar_row_relatorio(self, report: dict):
        ...
        def _on_check():
            self._toggle_selecao(report_id)
        chk = ctk.CTkCheckBox(row, text="", variable=sel_var, command=_on_check,
                               width=24, height=24, fg_color=THEME["primary"])
        chk.grid(row=0, column=0, sticky="w", padx=(SPACING["icon_gap"], SPACING["xs"]), pady=SPACING["item_gap"])

        icon_bg = ctk.CTkFrame(row, fg_color=THEME["kpi_blue_soft"], corner_radius=RADIUS["xs"],
                               width=32, height=32)
        icon_bg.grid(row=0, column=1, sticky="w", padx=(0, SPACING["icon_gap"]), pady=SPACING["item_gap"])
        icon_bg.pack_propagate(False)
        ...
        name_cell.grid(row=0, column=1, sticky="w", padx=(SPACING["icon_gap"], 0), pady=SPACING["item_gap"])
        ctk.CTkLabel(
            name_cell,
        ...
        actions.grid(row=0, column=4, sticky="e", padx=SPACING["icon_gap"], pady=SPACING["icon_gap"])

        for icon, tip, cmd in [
            (ICONS["view"], "Visualizar", lambda r=report: self._visualizar_relatorio(r)),
            (ICONS["download"], "Baixar",   lambda r=report: self._abrir_modal_download(r)),
    ...
    def _abrir_modal_gerar_relatorio(self, report_type=None):
        ...
        ctk.CTkLabel(wrapper, text="Template (Opcional)", font=themed_font("body_sm", "bold"),
                     text_color=THEME["text_secondary"]).pack(anchor="w")
        self._modal_template = ctk.CTkOptionMenu(
            wrapper, values=["Sem template"],
            font=themed_font("body"), height=36, corner_radius=RADIUS["input"],
        ...
        def _load_templates():
            try:
                res = self.controller_relatorio.listar_templates()
                if res.get("success"):
                    templates = res.get("data", [])
                    if isinstance(templates, dict):
                        templates = templates.get("templates", [])
                    vals = ["Sem template"] + [f"{t.get('id')} - {t.get('name', '')}" for t in templates if isinstance(t, dict)]
        ...
        ctk.CTkLabel(wrapper, text="Título do Relatório (Opcional)", font=themed_font("body_sm", "bold"),
                     text_color=THEME["text_secondary"]).pack(anchor="w")
        self._modal_nome = ctk.CTkEntry(wrapper, height=36, corner_radius=RADIUS["input"],
                                         font=themed_font("body"), placeholder_text="Ex: Fechamento Jan/2026")
        self._modal_nome.pack(fill="x", pady=(0, spacing("md")))

        ctk.CTkLabel(wrapper, text="Tipo de Relatório", font=themed_font("body_sm", "bold"),
                     text_color=THEME["text_secondary"]).pack(anchor="w")
        self._modal_tipo = ctk.CTkOptionMenu(
            wrapper,
            values=["geral", "estudante", "agendamentos", "intervencoes", "triagens", "estatisticas"],
            font=themed_font("body"), height=36, corner_radius=RADIUS["input"],
            fg_color=THEME["input_bg"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            dropdown_fg_color=THEME["surface"], dropdown_text_color=THEME["text"],
        ...
        if report_type:
            tipo_map = {
                "geral": "geral", "estudante": "estudante", "agendamentos": "agendamentos",
                "intervencoes": "intervencoes", "triagens": "triagens", "estatisticas": "estatisticas",
            }
            mapped = tipo_map.get(report_type, "geral")
            if mapped in self._modal_tipo.cget("values"):
                self._modal_tipo.set(mapped)

        self._modal_tipo.configure(command=lambda v: self._toggle_modal_campos_condicionais(v))

        self._modal_student_frame = ctk.CTkFrame(wrapper, fg_color=THEME["bg_alt"], corner_radius=RADIUS["md"])
        self._modal_student_frame.pack(fill="x", pady=(0, spacing("md")))
        self._modal_student_frame.pack_forget()

        ctk.CTkLabel(self._modal_student_frame, text="ID do Estudante",
                     font=themed_font("body_sm"), text_color=THEME["text_secondary"]).pack(anchor="w", padx=SPACING["card_pad"], pady=(SPACING["icon_gap"], 0))
        self._modal_student_id = ctk.CTkEntry(self._modal_student_frame, height=36, width=200,
                                               font=themed_font("body"), placeholder_text="Ex: 1")
        self._modal_student_id.pack(fill="x", padx=SPACING["card_pad"], pady=(0, SPACING["icon_gap"]))

        self._modal_date_frame = ctk.CTkFrame(wrapper, fg_color=THEME["bg_alt"], corner_radius=RADIUS["md"])
        ...
        ctk.CTkLabel(date_grid, text="Data Início", font=themed_font("body_sm"),
                     text_color=THEME["text_secondary"]).grid(row=0, column=0, sticky="w")
        self._modal_data_inicio = ctk.CTkEntry(date_grid, height=36, font=themed_font("body"), placeholder_text="YYYY-MM-DD")
        self._modal_data_inicio.grid(row=1, column=0, sticky="ew", padx=(0, SPACING["xs"]))

        ctk.CTkLabel(date_grid, text="Data Fim", font=themed_font("body_sm"),
                     text_color=THEME["text_secondary"]).grid(row=0, column=1, sticky="w")
        self._modal_data_fim = ctk.CTkEntry(date_grid, height=36, font=themed_font("body"), placeholder_text="YYYY-MM-DD")
        self._modal_data_fim.grid(row=1, column=1, sticky="ew", padx=(SPACING["xs"], 0))

        ctk.CTkLabel(wrapper, text="Formato de Saída", font=themed_font("body_sm", "bold"),
                     text_color=THEME["text_secondary"]).pack(anchor="w")
        ...
        for fmt, label in [("pdf", "PDF"), ("excel", "Excel"), ("csv", "CSV"), ("json", "JSON")]:
            ctk.CTkRadioButton(
                fmt_row, text=label, variable=self._modal_formato, value=fmt,
                font=themed_font("body_sm"), fg_color=THEME["primary"],
            ).pack(side="left", padx=(0, SPACING["icon_gap"]))

        btns = ctk.CTkFrame(wrapper, fg_color="transparent")
        btns.pack(fill="x", pady=(spacing("md"), 0))
        PrimaryButton(btns, text="Gerar Relatório", command=self._confirmar_gerar_relatorio,
                      width=160, height=40).pack(side="right", padx=(spacing("sm"), 0))
        GhostButton(btns, text="Cancelar", command=modal.destroy, width=120, height=40).pack(side="right")

        self._toggle_modal_campos_condicionais(self._modal_tipo.get())

    def _toggle_modal_campos_condicionais(self, tipo):
        show_student = tipo == "estudante"
        show_dates = tipo in ("agendamentos", "triagens", "intervencoes")
        if show_student:
            self._modal_student_frame.pack(fill="x", pady=(0, spacing("md")))
        else:
            self._modal_student_frame.pack_forget()
    ...
    def _confirmar_gerar_relatorio(self):
        try:
            ...
            if id_template:
                def fetch():
                    return self.controller_relatorio.gerar_relatorio_por_template(id_template, parametros)
            else:
                def fetch():
                    return self.controller_relatorio.gerar_relatorio({
            ...
            AsyncRunner.run(
                task=fetch,
                on_success=lambda res: self._on_generate_success(res),
    ...
    def _on_generate_success(self, res):
        if isinstance(res, dict) and res.get("success"):
            self._show_success(res.get("message", res.get("data", {}).get("name", "Relatório gerado.")))
            self._carregar_dados()
        else:
            msg = res.get("message", "Falha ao gerar relatório.") if isinstance(res, dict) else "Falha ao gerar relatório."
    ...
    def _abrir_modal_download(self, report):
        ...
        ctk.CTkLabel(wrapper, text=report.get("name", "Relatório"),
                     font=themed_font("body", "bold"), text_color=THEME["text"]).pack(anchor="w")
        ...
        for fmt_key, fmt_label, accent in formats:
            ctk.CTkButton(
                wrapper, text=f"Baixar {fmt_label}",
        ...
        GhostButton(wrapper, text="Cancelar", command=modal.destroy, width=120, height=38).pack(fill="x", pady=(SPACING["label_gap"], 0))

    def _baixar_relatorio_formato(self, report, formato):
        try:
            res = self.controller_relatorio.baixar_relatorio(report.get("id"))
            ...
            ext = {"pdf": ".pdf", "excel": ".xlsx", "csv": ".csv", "json": ".json"}.get(formato, ".txt")
            destino = filedialog.asksaveasfilename(
                initialfile=f"{nome}{ext}",
                defaultextension=ext,
                filetypes=[(fmt.upper(), f"*{ext}") for fmt in ["pdf", "excel", "csv", "json"]] + [("Todos", "*.*")],
            )
            if destino:
    ...
    def _visualizar_relatorio(self, report):
        ...
        rows = [
            ...
            ("Gerado em", str(report.get("generated_at", "—"))[:19] if report.get("generated_at") else "—"),
        ]
        for k, v in rows:
            r = ctk.CTkFrame(info, fg_color="transparent")
            r.pack(fill="x", padx=SPACING["card_pad"], pady=SPACING["grid_gap"] // 2)
            ctk.CTkLabel(r, text=k, font=themed_font("body_sm"), text_color=THEME["text_muted"]).pack(side="left")
            ctk.CTkLabel(r, text=str(v), font=themed_font("body", "bold"), text_color=THEME["text"]).pack(side="right")

        ctk.CTkLabel(wrapper, text="Conteúdo", font=themed_font("body", "bold"),
                     text_color=THEME["text"]).pack(anchor="w", pady=(SPACING["label_gap"], 0))

        content = ctk.CTkTextbox(wrapper, fg_color=THEME["bg_alt"], corner_radius=RADIUS["md"],
                                  font=themed_font("body"), text_color=THEME["text"])
        ...
        PrimaryButton(btns, text=f"{ICONS['download']} Baixar PDF",
                      command=lambda: self._baixar_relatorio_formato(report, "pdf"),
                      width=140, height=38).pack(side="right", padx=(spacing("sm"), 0))
        GhostButton(btns, text="Fechar", command=modal.destroy, width=120, height=38).pack(side="right")

    def _excluir_relatorio(self, report):
        rid = report.get("id")
        if not rid:
            return
        if not self._confirmar("Excluir este relatório?"):
            return
        try:
    ...
    def _criar_secao_templates(self):
        ...
        hdr.pack(fill="x")
        ctk.CTkLabel(
            hdr, text="Templates de Relatório",
            font=themed_font("body", "bold"),
            text_color=THEME["text"],
        ).pack(side="left")
        PrimaryButton(
            hdr, text=f"{ICONS['add']} Novo Template",
            command=self._abrir_modal_template,
            height=36, width=160, size="sm",
        ).pack(side="right")
        ...
        self._templates_filtro = ctk.CTkOptionMenu(
            ...
            font=themed_font("body"), height=34, width=160, corner_radius=RADIUS["input"],
            fg_color=THEME["primary_soft"], button_color=THEME["primary"],
            button_hover_color=THEME["primary_hover"],
            dropdown_fg_color=THEME["surface"], dropdown_text_color=THEME["text"],
            command=self._filtrar_templates,
        )
    ...
    def _carregar_templates(self):
        ...
        def fetch():
            return self.controller_relatorio.listar_templates(tipo=tipo_val)

        def _on_error(exc):
            self._show_error(f"Falha ao carregar templates.\n{exc}")

        AsyncRunner.run(
            task=fetch,
            on_success=lambda res: self._popular_lista_templates(res),
            on_error=_on_error,
            widget_ref=self,
        )

    def _filtrar_templates(self, _=None):
        self._carregar_templates()
    ...
    def _popular_lista_templates(self, res):
        ...
        if isinstance(res, dict):
            if res.get("success") is False:
                EmptyState(
                    self.templates_container, icon=ICONS["bolt"],
        ...
        if not templates:
            EmptyState(
                self.templates_container, icon=ICONS["empty"],
    ...
    def _abrir_modal_template(self, template=None):
        modal = BaseModal(self, title="Novo Template" if not template else "Editar Template", width=560, height=420)
        ...
        ctk.CTkLabel(wrapper, text="Nome", font=themed_font("body_sm"), text_color=THEME["text"]).pack(anchor="w")
        self._tmpl_nome = ctk.CTkEntry(wrapper, height=36, corner_radius=RADIUS["input"],
                                        font=themed_font("body"), fg_color=THEME["input_bg"],
        ...
        ctk.CTkLabel(wrapper, text="Tipo", font=themed_font("body_sm"), text_color=THEME["text"]).pack(anchor="w")
        ...
        ctk.CTkLabel(wrapper, text="Configuração (JSON opcional)", font=themed_font("body_sm"), text_color=THEME["text"]).pack(anchor="w")
        self._tmpl_config = ctk.CTkTextbox(wrapper, height=100, corner_radius=RADIUS["input"],
                                            font=themed_font("body"), fg_color=THEME["input_bg"],
                                            border_width=1, border_color=THEME["input_border"])
        ...
        btns = ctk.CTkFrame(wrapper, fg_color="transparent")
        btns.pack(fill="x", pady=(spacing("md"), 0))
        PrimaryButton(btns, text="Salvar", command=lambda: self._salvar_template(modal, template),
                      width=120, height=40).pack(side="right", padx=(spacing("sm"), 0))
        GhostButton(btns, text="Cancelar", command=modal.destroy, width=120, height=40).pack(side="right")
    ...
    def _salvar_template(self, modal, template=None):
        ...
        def fetch():
            if template:
                return self.controller_relatorio.atualizar_template(template.get("id"), dados)
            return self.controller_relatorio.criar_template(dados)

        def _on_error(exc):
            self._show_error(f"Falha ao salvar template.\n{exc}")
    ...
    def _preview_template(self, template):
        def fetch():
            return self.controller_relatorio.listar_relatorios_filtrados(
        ...
        def on_error(exc):
            self._show_error(f"Falha no preview.\n{exc}")
    ...
    def _excluir_template(self, template):
        ...
        def fetch():
            return self.controller_relatorio.deletar_template(template.get("id"))

        def _on_error(exc):
            self._show_error(f"Falha ao excluir template.\n{exc}")

        AsyncRunner.run(
            task=fetch,
            on_success=lambda res: self._on_template_delete(res),
            on_error=_on_error,
            widget_ref=self,
        )
...
def _bind_clickable_recursive(widget, on_click):
    from ser_pleno.presentation.components.ui_components import _CLICKABLE_EXCLUDE
