...
class _AlertaRow(ctk.CTkFrame):
    ...
    _PRIORITY_ICON  = {0: "ðŸŸ¡", 1: "ðŸŸ ", 2: "ðŸ”´", 3: "ðŸ”´"}
...
class NotificationPanel(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        icon_txt = "ðŸ¤" if is_ajuda else "ðŸ””"
        ...
        if not self.notificacoes:
            EmptyState(
                lst, icon="ðŸ“­", title="Sem notificaçÓµes",
...
class DashboardFrame(ctk.CTkScrollableFrame):
    ...
    def _render_kpis(self, data):
        ...
        kpis = [
            ("Atendimentos Hoje", str(data.get("appointments_today", 0)),
             "ðŸ‘¥", THEME["kpi_blue"],   THEME["kpi_blue_soft"],  "Atendimentos marcados"),
            ("Vagas Disponíveis", str(data.get("available_slots", 0)),
             "ðŸ“…", THEME["kpi_green"],  THEME["kpi_green_soft"], "Horários livres"),
            ("Alertas Ativos",    str(data.get("alerts", 0)),
             "ðŸ””", THEME["kpi_red"],    THEME["kpi_red_soft"],   "Requerem atenção"),
            ("Total de Estudantes", str(data.get("total_students", 0)),
             "ðŸŽ“", THEME["kpi_violet"], THEME["kpi_violet_soft"],"Alunos cadastrados"),
            ("Humor Médio",
    ...
    def _criar_card_chart(self):
        self.chart_card = Card(self.left_col, title="ðŸ“ˆ  Humor dos Estudantes — últimos 30 dias")
    ...
    def _atualizar_secao_agenda(self, data):
        ...
        card = Card(
            self.left_col, title="ðŸ“…  Próximos Atendimentos",
        )
    ...
    def _atualizar_secao_alertas(self, data):
        ...
        card = Card(
            self.right_col, title=f"ðŸ”´  Estudantes em Alerta",
        )
        card.pack(fill="x", pady=(0, 14))
    ...
    def _atualizar_secao_bem_estar(self, data):
        card = Card(self.right_col, title="ðŸ’š  Bem-Estar por Dimensão")
        card.pack(fill="x", pady=(0, 14))

        be = data.get("bem_estar_dimensions", {})
        dims = [
            ("ðŸ“  Acadêmico", be.get("academico", 0) / 5, THEME["primary"], THEME["primary_soft"]),
            ("ðŸ’—  Emocional",  be.get("emocional", 0) / 5, "#EC4899", "#FCE7F3"),
            ("ðŸ‘¥  Social",    be.get("social",    0) / 5, THEME["success"], THEME["success_soft"]),
        ]
    ...
    def _draw_chart(self, humor_history=None):
        ...
        # Órea preenchida (gradiente simulado com polygon)
        poly_pts = []
        for x, y in coords:
            poly_pts += [x, y]
    ...
    def _abrir_notificacoes_alertas(self):
        ...
        NotificationPanel(
            self, "NotificaçÓµes de Alerta", notifs, "alerta",
            on_mark_read=self._marcar_lida,
            on_mark_all_read=self._marcar_todas_lidas,
        )
    ...
    def _humor_emoji(media) -> str:
        if media is None: return "ðŸ˜"
        if media < 2.0:   return "ðŸ˜¢"
        if media < 3.0:   return "ðŸ˜•"
        if media < 4.0:   return "ðŸ˜Š"
        return "ðŸ˜„"
