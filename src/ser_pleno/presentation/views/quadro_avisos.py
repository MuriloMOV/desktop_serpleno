...
class PublicacaoModal(ctk.CTkToplevel):
    ...
    def _build(self, W, H):
        ...
        ctk.CTkLabel(ts, text="Formulário Ó  esquerda Â· pré-visualização Ó  direita",
                     font=font(size=10),
                     text_color=Q["text_muted"]).pack(anchor="w")

        # Botão fechar
        ctk.CTkButton(bi, text="âœ•", width=32, height=32, corner_radius=8,
    ...
    def _build_form(self, parent):
        ...
        self.f_titulo   = FormField(scroll, "ðŸ“  Título", placeholder="Título da publicação")
        self.f_titulo.pack(fill="x", pady=(0, 10))

        self.f_conteudo = FormField(scroll, "ðŸ“„  Conteúdo",
        ...
        self.f_categoria = FormField(row1, "ðŸ·  Categoria",
                                     values=["informativo","aviso","aula","urgente","evento"],
                                     initial="informativo")
        self.f_categoria.grid(row=0, column=0, sticky="ew", padx=(0, 6))

        self.f_autor = FormField(row1, "ðŸ‘¤  Autor", placeholder="Nome do autor")
        ...
        self.f_local = FormField(row2, "ðŸ“  Local", placeholder="Ex: Auditório")
        self.f_local.grid(row=0, column=0, sticky="ew", padx=(0, 6))

        self.f_link  = FormField(row2, "ðŸ”—  Link externo", placeholder="https://...")
        ...
        self.f_data_ag = FormField(row3, "ðŸ“…  Agendamento", placeholder="YYYY-MM-DD")
        self.f_data_ag.grid(row=0, column=0, sticky="ew", padx=(0, 6))

        self.f_horario_evento = FormField(row3, "ðŸ•’  Horário evento",
        ...
        ctk.CTkButton(
            blk_btns, text="ï¼‹  Adicionar Bloco",
    ...
    def _build_preview(self, parent):
        ...
        ctk.CTkFrame(parent, height=1, fg_color=Q["divider"]).pack(fill="x", padx=16)
        ...
        prev_card.pack(fill="both", expand=True, padx=16, pady=12)
    ...
    def _render_block_editors(self, cols: int, existing: list[dict]):
        ...
        for i in range(cols):
            ...
            ctk.CTkLabel(hdr, text=f"  Bloco #{i + 1}",
                         font=font(size=11, weight="bold"),
                         text_color=Q["accent"]).pack(side="left", pady=6)
    ...
    def _update_preview(self):
        ...
        if layout == "single":
            ...
            for i in range(min(cols, 4)):
                ...
                f.grid(row=0, column=i, padx=4, pady=4, sticky="n")
...
class QuadroAvisosFrame(ctk.CTkFrame):
    def __init__(self, master, app):
        ...
        self.lista.pack(fill="both", expand=True, padx=28, pady=(0, 20))
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(left, text="Gerencie publicaçÓµes e comunicados do sistema",
                     font=font(size=13),
                     text_color=Q["text_muted"]).pack(anchor="w", pady=(2, 0))

        ctk.CTkButton(
            bar, text="ï¼‹  Novo Aviso",
    ...
    def carregar_avisos_async(self):
        ...
        ctk.CTkLabel(self.lista,
                     text="â³  Carregando publicaçÓµes...",
                     font=font(size=13),
    ...
    def _on_load_success(self, res):
        ...
        if not posts:
            ctk.CTkLabel(self.lista,
                         text="ðŸ“­  Nenhuma publicação encontrada",
                         font=font(size=13),
    ...
    def _on_load_error(self, e):
        self._limpar_lista()
        ctk.CTkLabel(self.lista,
                     text=f"âš   Erro ao carregar avisos: {e}",
                     font=font(size=12),
    ...
    def _criar_card(self, aviso_id, titulo, descricao, autor, data, categoria="informativo"):
        ...
        for icon, val in [("ðŸ‘¤", autor), ("ðŸ•’", data)]:
            if val:
