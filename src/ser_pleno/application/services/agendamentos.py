...
# InstÓ¢ncia global do serviço de autenticação
_auth_service = None

def set_auth_service(auth_service):
    """Define o serviço de autenticação global para usar nas requisiçÓµes API"""
...
class ServicoAgendamento:
    ...
    def _get_headers(self):
        """Retorna os headers para as requisiçÓµes API, incluindo CSRF token"""
        headers = {
    ...
    def adicionar_horario_disponibilidade(self, horario):
        """Adiciona um novo horário Ó  tabela de disponibilidade usando a API do Serpleno Web"""
        try:
