...
class ServicoAutenticacao:
    ...
    def _try_establish_session(self, usuario, senha):
        ...
        Isso é necessário para que as requisiçÓµes subsequentes funcionem com @login_required.
        """
        try:
            login_url = f"{self.API_BASE_URL}/api/v1/serpleno/auth/login/"
            response = self.session.post(
    ...
    def get_session(self):
        """Retorna a sessão HTTP para uso em outras requisiçÓµes"""
        return self.session
    
    def get_headers(self):
        """Retorna headers com CSRF token para requisiçÓµes POST/PUT/DELETE"""
        headers = {
