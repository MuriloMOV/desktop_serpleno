"""
Serviço de OrientaçÓµes para o Desktop CustomTkinter
...
from ser_pleno.infrastructure.api.client import api

logger = logging.getLogger(__name__)


class ServicoOrientacoes:
    """Serviço para gerenciar orientaçÓµes via API do SerPleno Web"""
    ...
    PRESETS = {
        "study_routine": {
            ...
            "components": [
                ...
                {"id": "p2", "type": "textarea", "label": "Passos/RecomendaçÓµes"},
        ...
        "emotional_support": {
            ...
            "components": [
                {"id": "p4", "type": "text", "label": "Sintomas/ObservaçÓµes"},
                {
                    "id": "p5",
                    "type": "checkbox",
                    "label": "Encaminhar para Atendimento",
                },
                {"id": "p6", "type": "textarea", "label": "SugestÓµes de Autocuidado"},
            ],
        },
        "follow_up": {
    ...
    def listar_orientacoes(
        ...
        """
        Lista orientaçÓµes com filtros opcionais
        ...
        try:
            ...
            if result.get("success") and result.get("data", {}).get("orientations"):
                logger.info("OrientaçÓµes obtidas do banco local")
                return result
        except Exception as e:
            logger.error(f"Erro ao buscar orientaçÓµes no banco local: {e}")
        ...
        try:
            ...
            logger.exception(f"Erro ao listar orientaçÓµes: {e}")
            return {
                "success": True,
                "data": {"orientations": [], "pagination": {"page": 1, "total": 0}},
            }
    ...
    def obter_estatisticas(self, id_estudante: Optional[int] = None) -> Dict[str, Any]:
        """
        Obtém estatísticas das orientaçÓµes
        ...
        if not self._should_use_api():
            logger.info(
                "Modo independente: usando banco local para estatísticas de orientaçÓµes"
            )
    ...
    def _local_listar_orientacoes(
        ...
        try:
            ...
            logger.info(f"Encontradas {total} orientaçÓµes no banco de dados local")
            connection.close()
            ...
            logger.error(f"Erro ao listar orientaçÓµes locais: {e}")
            return {
                "success": True,
                "data": {"orientations": [], "pagination": {"page": 1, "total": 0}},
            }
    ...
    def _local_criar_orientacao(self, dados: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Adiciona Ó  fila de sincronização
            try:
...
# InstÓ¢ncia global para fácil acesso
