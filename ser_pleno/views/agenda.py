...
class AgendaFrame(ctk.CTkScrollableFrame):
    ...
    def renderizar_grid(self, container, mapa_dados):
        ...
        for idx, hora in enumerate(self.horarios_base):
            ...
            cell = ctk.CTkFrame(
                ...
                border_width=1,
                fg_color=blend_color("#7E22CE", 0.06) if ocupado else blend_color("#16A34A", 0.06),
                border_color=blend_color("#7E22CE", 0.22) if ocupado else blend_color("#16A34A", 0.22),
            )
            ...
            ctk.CTkLabel(cell, text=hora, font=themed_font("body", "bold"), text_color=cor_destaque).pack(pady=(10, 0))
            nome_display = info['nome'] if ocupado else "Disponível"
            lbl_nome = ctk.CTkLabel(cell, text=nome_display, font=themed_font("overline", "bold" if ocupado else "normal"), text_color=cor_destaque, wraplength=110)
            lbl_nome.pack()
    ...
    def abrir_modal_agendamento(self, hora, info=None):
        ...
        ctk.CTkLabel(container, text="Editar Agendamento" if info else "Novo Agendamento", font=themed_font("h3", "bold"), text_color=THEME["text"]).pack(anchor="w", pady=(0, 16))

        ctk.CTkLabel(container, text="Horário", font=themed_font("caption", "bold"), text_color=THEME["text_muted"]).pack(anchor="w", pady=(0, 5))
        combo_hora = ctk.CTkComboBox(container, values=self.horarios_base, width=380, height=44, fg_color="white", border_color=THEME["border"], button_color=THEME["bg_alt"], button_hover_color=THEME["border"], dropdown_fg_color="white")
        combo_hora.set(hora)
        combo_hora.pack(pady=(0, 12))

        ctk.CTkLabel(container, text="Estudante", font=themed_font("caption", "bold"), text_color=THEME["text_muted"]).pack(anchor="w", pady=(0, 5))
        combo_estudante = ctk.CTkComboBox(container, values=list(self.mapa_estudantes.keys()), width=380, height=44, fg_color="white", border_color=THEME["border"], button_color=THEME["bg_alt"], button_hover_color=THEME["border"], dropdown_fg_color="white")
        if info:
        ...
        ctk.CTkLabel(container, text="Status", font=themed_font("caption", "bold"), text_color=THEME["text_muted"]).pack(anchor="w", pady=(0, 5))
        combo_status = ctk.CTkComboBox(container, values=["Agendado", "Realizado", "Cancelado", "Faltou"], width=380, height=44, fg_color="white", border_color=THEME["border"], button_color=THEME["bg_alt"], button_hover_color=THEME["border"], dropdown_fg_color="white")
        if info:
        ...
        ctk.CTkLabel(container, text="Observações", font=themed_font("caption", "bold"), text_color=THEME["text_muted"]).pack(anchor="w", pady=(0, 5))
        txt_obs = ctk.CTkTextbox(container, height=100, fg_color="white", border_width=1, border_color=THEME["border"], corner_radius=RADIUS["input"])
        if info:
        ...
        if info:
            GhostButton(footer, text="Remover", command=lambda: self.remover_agendamento(info['id_agendamento'], modal), width=110).pack(side="left")

        PrimaryButton(footer, text="Salvar", command=lambda: self.salvar_agendamento(modal, combo_hora.get(), combo_estudante.get(), combo_status.get(), txt_obs.get("1.0", "end-1c"), info), width=120).pack(side="right")
        GhostButton(footer, text="Cancelar", command=modal.destroy, width=110).pack(side="right", padx=10)
    ...
    def criar_container_agenda_dia(self):
        ...
        ctk.CTkLabel(card.body, text="Horários do Dia", font=themed_font("h3", "bold"), text_color=THEME["text"]).pack(anchor="w", pady=(0, 8))
    ...
    def criar_container_proxima_semana(self):
        ...
        ctk.CTkLabel(header, text="Próxima Semana", font=themed_font("h3", "bold"), text_color=THEME["text"]).pack(side="left")
        self.lbl_subtitulo_semana = ctk.CTkLabel(header, text="", font=themed_font("body"), text_color=THEME["text_muted"])
    ...
    def abrir_modal_gestao(self):
        ...
        ctk.CTkLabel(container, text="Horários Ativos", font=themed_font("h3", "bold"), text_color=THEME["text"]).pack(pady=(0, 12))

        frame_lista = ctk.CTkScrollableFrame(container, width=340, height=280, fg_color=THEME["bg_alt"], corner_radius=RADIUS["md"])
        ...
        def atualizar_lista_modal():
            ...
            for h in self.horarios_base:
                row = ctk.CTkFrame(frame_lista, fg_color="white", corner_radius=RADIUS["sm"], border_width=1, border_color=THEME["border"])
                row.pack(fill="x", pady=4, padx=4)
                ctk.CTkLabel(row, text=h, font=themed_font("body", "bold")).pack(side="left", padx=12, pady=8)
                GhostButton(row, text="Remover", command=lambda x=h: self.remover_horario_disponibilidade(x, atualizar_lista_modal), width=90).pack(side="right", padx=8, pady=6)

        atualizar_lista_modal()

        ctk.CTkLabel(container, text="Novo Horário (HH:MM)", font=themed_font("body", "bold"), text_color=THEME["text"]).pack(anchor="w", pady=(14, 5))
        new_h = ctk.CTkEntry(container, width=140, height=36, corner_radius=RADIUS["sm"], border_width=1, border_color=THEME["border"])
        new_h.pack(pady=(0, 8))
        ...
        PrimaryButton(container, text="Adicionar à Grade", command=lambda: self.adicionar_horario_disponibilidade(new_h.get(), atualizar_lista_modal)).pack()
